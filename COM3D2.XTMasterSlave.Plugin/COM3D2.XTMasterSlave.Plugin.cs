...
namespace CM3D2.XtMasterSlave.Plugin
{
    ...
    public class XtMasterSlave : ExPluginBase
    {
        ...
        class OhMyGizmo : GizmoRender
        {
            ...
            public void BkupPos()
            {
                _backup_pos = this.position;
            }

            public void BkupRot()
            {
                _backup_rot = this.rotation;
            }

            public void BkupPosAndRot()
            {
                this.BkupPos();
                this.BkupRot();
            }
            ...
            public void BkupPosAndRotU1()
            {
                _backup_pos_u1 = this.transform.position;
                _backup_rot_u1 = this.transform.rotation;
            }

            public void BkupPosAndRotLocalU1()
            {
                _backup_posLocal_u1 = this.transform.localPosition;
                _backup_rotLocal_u1 = this.transform.localRotation;
            }
            ...
            public OhMyGizmo()
            {
                ...
                if (_fi_beSelectedType == null)
                    _fi_beSelectedType = typeof(GizmoRender).GetField("beSelectedType",
                        BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);

                if (_fi == null)
                    _fi = typeof(GizmoRender).GetField("is_drag_",
        ...
        public class MsLinkConfig
        {
            ...
            public bool chkIkSpCustomR_v2()
            {
                return doIKTargetMHandSpCustom && doIKTargetMHandSpCustom_v2 &&
                       doIKTargetMHandSpR_TgtChar != ATgtChar.None;
            }

            public bool chkIkSpCustomL_v2()
            {
                return doIKTargetMHandSpCustom && doIKTargetMHandSpCustom_v2 &&
                       doIKTargetMHandSpL_TgtChar != ATgtChar.None;
            }
            ...
            // v0026
            public bool
                doIK159NewPointToDef = false; //v5.0ギリギリ使えるレベルに //v4.0β※com1.17で不具合が出る true; // 159以降の新IKをデフォルトに使うか
        ...
        public static MsLinkConfig[] cfgs = new MsLinkConfig[MAX_PAGENUM]
            { new MsLinkConfig(), new MsLinkConfig(), new MsLinkConfig(), new MsLinkConfig(), new MsLinkConfig() };
        ...
        public class v3OffsetsV2
        {
            ...
            public Vector3 v3StackOffset
            {
                get { return v3o.v3StackOffset; }
                set { v3o.v3StackOffset = value; }
            }

            public Vector3 v3StackOffsetRot
            {
                get { return v3o.v3StackOffsetRot; }
                set { v3o.v3StackOffsetRot = value; }
            }
        ...
        public class v3Offsets
        {
            ...
            */
			...
			public Vector3 v3StackOffset2Bip(Maid slave, bool trans)
			{
				...
				var sTr =
 BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, (!slave.boMAN ? "Bip01" : "ManBip"), true);
        ...
        public static v3Offsets[] v3ofs = new v3Offsets[MAX_PAGENUM]
            { new v3Offsets(), new v3Offsets(), new v3Offsets(), new v3Offsets(), new v3Offsets() };

        public class YotogiConfig
        {
            //絶頂判定用
            public string[] sMensZeccyouMotion =
            {
                /*"_zeccyou_m_once_",*/
                "_shasei_naka_m_once_", "_shasei_soto_m_once_", "_shasei_kao_", "_shasei_kuti_", "2ana_shasei_.*_m2"
            }; /*"_seikantaizeccyou_m_once_",*/

            public string[] sMensZeccyouAfterMotion =
                { "_zeccyougo_m", "_shaseigo_naka_m", "_shaseigo_soto_m", "shaseigo" };

            public string[] sMensSexMotion = new string[]
            {
                "seijyoui_", "kouhaii_", "sokui_", "kijyoui_", "haimen_", "ekiben_", "sex_", "manguri_", "ritui_",
                "seijyouia_", "kouhaiia_", "sokuia_", "kijyouia_", "haimena_", "ekibena_", "sexa_", "manguria_",
                "rituia_", "ran3p_2ana", "ran4p_", "taimenkijyoui"
            };

            public string[] sMensKissMotion = new string[] { "sixnine", "^(?!.*fera).*kiss.*(?!.*fera)", "kunni", };

            public string[] sMensUkeMotion = new string[]
            {
                "fera", "_ir_", "siriname", "tikubiname", "tekoki", "paizuri", "kijyoui", "ran3p_housi", "harem_housi",
                "mp_arai_", "asikoki_"
            }; //_ir_イラマ
        ...
        static FieldInfo fiml_ = typeof(TBodySkin).GetField("m_listManAlphaMat",
        ...
        static bool NeedHitScaleCalc = typeof(TBodySkin).GetField("m_trMaid",
        ...
        public static void PreUpdateSc(TBodyHit tbh, float scale)
        {
            for (int i = 0; i < tbh.spherelist.Count; i++)
            {
                ...
                if (tbh.spherelist[i].tPtr != null)
                    tbh.spherelist[i].tPtr.localScale = new Vector3(tbh.spherelist[i].len,
                        tbh.spherelist[i].tPtr.localScale.y, tbh.spherelist[i].tPtr.localScale.z);
#endif
        ...
        public static void UpdateHitScale(Maid maid, float bodyScale, MsLinkConfig mcfg)
        {
            ...
            UpdateHitScale_boneh3(maid, mcfg.Scale_HitCheckDetail_Thigh, mcfg.Scale_HitCheckDetail_Hip,
                mcfg.Scale_HitCheckDetail_Momo);
        }
                ...
                if (s != null)
                {
                    if (s.bonehair2 != null)
                    {
                        ...
                        if (IkXT.IsIkCtrlO132)
                        {
                            ...
                            list = s.bonehair2.GetType().GetField("m_listCollders",
        ...
        public static void UpdateHitScale_boneh3(Maid maid, float scaleReg, float scaleHip, float scaleVag)
        {
            ...
            for (int n = 0; n < body.goSlot.Count; n++)
            {
                ...
                if (s != null)
                {
                    if (s.bonehair3 != null)
                    {
                        DynamicSkirtBone bone = s.bonehair3.GetType()
                            .GetField("m_SkirtBone", BindingFlags.NonPublic | BindingFlags.Instance)
                ...
                if (tBodySkin.morph != null)
                {
                    for (int j = 0; j < tBodySkin.morph.BoneNames.Count; j++)
                    {
                        if (tBodySkin.morph.BoneNames[j].ToLower().Contains("tamabukuro"))
                        {
                            ...
                            tBodySkin.morph.GetType().InvokeMember("m_bDut",
        ...
        public class keepSlaveInfo
        {
            ...
            public bool CheckMoved(Maid m, int pageN)
            {
                ...
                {
                    debugPrintConsole(lastMotionFN + lastPos + " != " + m.body0.LastAnimeFN +
            ...
            */
            ...
            for (int i = 0; i < cfgs.Length; i++)
            {
                ...
                if (i == 0)
                    ...
                    cfgs[i] = SharedConfig.ReadConfig<MsLinkConfig>("Config-" + (i + 1).ToString(),
                        pathConfig(PluginCfgFN));
            }
            ...
            VYMModule.VymModule.cfg =
                PluginExt.SharedConfig.ReadConfig<VYMModule.VymModule.VibeYourMaidConfig>("VYMAsset",
                    pathConfig(YotogiCfgFN));

            // v0027
            // ボイステーブルをini→CSVに
            VymModule.voiceLegacy =
                SharedConfig.ReadConfig<VymModule.VoiceTableLegacy>("VYMAsset", pathConfig(YotogiCfgFN));
        ...
        static Rect rc_stgw =
            new Rect(UnityEngine.Screen.width + WIDTH_DPOS, /*UnityEngine.Screen.height + HEIGHT_DPOS*/
                UnityEngine.Screen.height / 720 * 35, GUI_WIDTH, GUI_HIGHT);
        ...
        void WindowCallback(int id)
        {
            try
            {
                ...
                {
                    XtMs2ndWnd.SaveWindowCallback_proc(id, this, ref showWndMode, ref _pageNum, _MSlinks, cfgs, v3ofs,
                        out _WinprocPhase);
                ...
                if (needUpdate || Time.frameCount % 24 == 0) //更新は24フレーム毎
                {
                    foreach (var v in dicManAlpha.ToArray())
                    {
                        if (_MSlinks.Any(link => link.curMaster_ == v.Key
                                                 || (link.maidKeepSlaveYotogi &&
                                                     link.do_master == v.Key))) //どこかのページでマスターしてるか
                        {
        ...
        public class MsLinks
        {
            ...
            {
                curMaster_ = (mdMaster_No >= 0) ? mdMasters[mdMaster_No].mem : null;
            }

            public void FixSlave()
            {
                curSlave_ = (mdSlave_No >= 0) ? mdSlaves[mdSlave_No].mem : null;
            }
            ...
            public bool CheckPlayMaids()
            {
                return (mdSlaves.Count > mdSlave_No) && (mdSlave_No > 0 || !bIsYotogiScene)
                                                     && mdSlaves[0].mem && !mdSlaves[0].mem.boMAN &&
                                                     mdSlaves[mdSlave_No].mem;
            }
            ...
            public void lateUpdate()
            {
                ...
                if (cfgs[this.num_].doFaceSync && !this.mdSlaves[0].mem.boMAN && this.mdSlave_No > 0)
                {
                    ...
                    {
                        //表情同期２
                        FaceBlend2Sync(this.mdSlaves[0].mem.body0.Face.morph,
                            this.mdSlaves[this.mdSlave_No].mem.body0.Face.morph, true);
                    }
                }
            ...
            */
			...
			class IkMini
			{
				...
				public void IkProc1(Transform shoulder, Transform elbow, Transform hand, Vector3 tgt, Vector3 vechand_offset)
				{
					...
					shoulder.rotation =
 Quaternion.FromToRotation(vectgt.normalized, tgt.normalized) * shoulder.rotation;
					return;
				}
				...
				public void IkProc2(Transform shoulder, Transform elbow, Transform hand, Vector3 tgt, Vector3 vechand_offset)
				{
                    ...
                    */
					...
					shoulder.rotation =
 Quaternion.FromToRotation(vechnd.normalized, tgtv.normalized) * shoulder.rotation;


					var q =
 Quaternion.FromToRotation((elbow.position - shoulder.position).normalized, vechnd.normalized) * shoulder.rotation;
            ...
            public bool lateHandsAtcpProc2(Maid slave)
            {
                ...
                if (!p_mscfg.doIKTargetMHandSpCustom)
                {
                    if (master && p_mscfg.doIKTargetMHand) // v5.0
                    {
                        if (!p_mscfg.doIK159NewPointToDef && p_mscfg.doIK159RotateToHands)
                        {
                            ...
                            Transform trh0 = BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform,
                                GetHandBnR(master), true);
                            Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                                GetHandBnR(slave), true);
                            if (!IsHookActive && !IkXT.IsNewPointIK(slave, "右手") &&
                                IkXT._inst.IKCmoUpdate(slave.body0, trh, p_v3of.v3HandROffset, "右手"))
                            {
                            ...
                            trh0 = BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, GetHandBnL(master),
                                true);
                            trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnL(slave),
                                true);
                            if (!IsHookActive && !IkXT.IsNewPointIK(slave, "左手") &&
                                IkXT._inst.IKCmoUpdate(slave.body0, trh, p_v3of.v3HandLOffset, "左手"))
                            {
                ...
                //if (!doMasterSlave || cfgs[num_].doStackSlave_PosSyncMode)
                {
                    ...
                    //if (p_v3of.v3HandROffsetRot != Vector3.zero)
                    {
                        ...
                        if (p_mscfg.doIKTargetMHandSpR_TgtChar != ATgtChar.None
                            && ikp.tgtMaidR && ikp.tgtHandR_AttachSlot >= 0 &&
                            !string.IsNullOrEmpty(ikp.tgtHandR_AttachName))
                        {
                            ...
                            if (p_mscfg.doIKTargetMHandSpCustomAltRotR)
                            {
                                ...
                                {
                                    if (iks.tgtMaidR != null && iks.tgtMaidR.body0 != null &&
                                        iks.tgtMaidR.body0.goSlot[iks.tgtHandR_AttachSlot].morph != null)
                                    {
                                        Vector3 vector2;
                                        Quaternion rotation2;
                                        iks.tgtMaidR.body0.goSlot[iks.tgtHandR_AttachSlot].morph
                                            .GetAttachPoint(iks.tgtHandR_AttachName, out vector2, out rotation2);
                                        if (p_mscfg.doIKTargetMHandSpCustom_v2 && !p_mscfg.doIK159NewPointToDef)
                                        {
                                            if (!IsHookActive && IkXT._inst.IKCmoUpdate(slave.body0, trh,
                                                    p_v3of2.v3HandROffset, "右手"))
                                            {
                                                // IK適用済みのはずなので解除
                                                slave.body0._ikp().tgtMaidR = null;
                                                slave.body0._ikp().tgtHandR = null;
                                        ...
                                        */
                                        // IKリミットチェック
                                        var rm = trh.gameObject.transform.gameObject
                                            .GetComponent<RootMotion.FinalIK.RotationLimit>();
                                        if (rm)
                                        {
                                            bool flag;
                            ...
                            else if (p_mscfg.doIKTargetMHandSpCustom_v2) // v5.0
                            {
                                    ...
                                    iks.tgtMaidR.body0.goSlot[iks.tgtHandR_AttachSlot].morph != null)
                                {
                                    ...
                                    Quaternion rotation;
                                    iks.tgtMaidR.body0.goSlot[iks.tgtHandR_AttachSlot].morph
                                        .GetAttachPoint(iks.tgtHandR_AttachName, out vector, out rotation);
                            ...
                            else if (doMasterSlave && p_mscfg.doIK159NewPointToDef &&
                                     p_mscfg.doIK159RotateToHands) // v5.0
                            {
                                trh.localRotation *= Quaternion.Euler(p_v3of2.v3HandROffsetRot);
                                //slave.body0.AutoTwist();
                                needTwist = true;
                            }
                            else if (!doMasterSlave || !cfgs[num_].doStackSlave ||
                                     !cfgs[num_].doIKTargetMHand) // ボーンリンク中は二重補正になるのでしない
                            {
                        ...
                        else if (ikp.tgtHandR && p_mscfg.doIKTargetMHandSpR_TgtChar != ATgtChar.None) // v5.0
                        {
                            ...
                            bool reset = !IsHookActive &&
                                         IkXT._inst.IKCmoUpdate(slave.body0, trh, p_v3of2.v3HandROffset, "右手");
                    ...
                    //if (p_v3of.v3HandLOffsetRot != Vector3.zero)
                    {
                        ...
                        if (p_mscfg.doIKTargetMHandSpL_TgtChar != ATgtChar.None
                            && ikp.tgtMaidL && ikp.tgtHandL_AttachSlot >= 0 &&
                            !string.IsNullOrEmpty(ikp.tgtHandL_AttachName))
                        {
                            // 左手
                            Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                                GetHandBnL(slave), true);
                            //bkupHandRotL = true;
                            ...
                            if (p_mscfg.doIKTargetMHandSpCustomAltRotL)
                            {
                                ...
                                {
                                    if (iks.tgtMaidL != null && iks.tgtMaidL.body0 != null &&
                                        iks.tgtMaidL.body0.goSlot[iks.tgtHandL_AttachSlot].morph != null)
                                    {
                                        Vector3 vector;
                                        Quaternion rotation;
                                        iks.tgtMaidL.body0.goSlot[iks.tgtHandL_AttachSlot].morph
                                            .GetAttachPoint(iks.tgtHandL_AttachName, out vector, out rotation);
                                        if (p_mscfg.doIKTargetMHandSpCustom_v2 && !p_mscfg.doIK159NewPointToDef)
                                        {
                                            if (!IsHookActive && IkXT._inst.IKCmoUpdate(slave.body0, trh,
                                                    p_v3of2.v3HandLOffset, "左手"))
                                            {
                                                // IK適用済みのはずなので解除
                                                slave.body0._ikp().tgtMaidL = null;
                                                slave.body0._ikp().tgtHandL = null;
                        ...
                        else if (ikp.tgtHandL && p_mscfg.doIKTargetMHandSpL_TgtChar != ATgtChar.None) // v5.0
                        {
                            Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                                GetHandBnL(slave), true);
                            bool reset = !IsHookActive &&
                                         IkXT._inst.IKCmoUpdate(slave.body0, trh, p_v3of2.v3HandLOffset, "左手");
            ...
            public void linkProc()
            {
                ...
                //回想・夜伽モード中のサブメイドキープ
                if (XtMasterSlave.IsKeepScene() /*(vIsKaisouScene || bIsYotogiScene)*/ && maidKeepSlaveYotogi &&
                    !Scc1_MasterMaid && cfg_.doKeepSlaveYotogi
                        ...
                        if (mdSlave_No < 0 || mdSlaves[mdSlave_No].mem != maidKeepSlaveYotogi)
                        {
                            ...
                            if (!loaded)
                            {
                                if (!GameMain.Instance.CharacterMgr.IsBusy() &&
                                    GameMain.Instance.CharacterMgr.GetMaid(0) != null)
                                {
                                    if (LoadMaid(maidKeepSlaveYotogi))
                                    {
                                        debugPrintConsole("きーぷめいどろーど完了・・・@link " + num_ + " mid:" +
                                                          do_master.GetInstanceID());
                ...
                //ボイスプレイ
                if (cfg_.doVoiceAndFacePlay &&
                    (bIsYotogiScene || (bIsVymPlg && cfg_.doVoiceAndFacePlayOnVYM) || cfg_.doManualVfPlay) &&
                    CheckPlayMaids())
                {
                    if (!slave.boMAN)
                    {
                        ...
                        if (cfg_.doManualVfPlay)
                        {
                            ...
                            {
                                if (slave.AudioMan.audiosource.loop || (!slave.AudioMan.audiosource.loop &&
                                                                        !slave.AudioMan.audiosource.isPlaying))
                                {
                                    //音声割り込みタイミングで絶頂させる
                        ...
                        VYMModule.VymModule.MaidVoicePlay(slave, iExcite, stat, motionChanged, !cfg_.doFaceSync,
                            !cfg_.doVoiceDisabled, (!bIsYotogiScene && bIsVymPlg),
                        ...
                        if (cfg_.doZecchoKeiren && (stat == AnimeState.State.zeccho || this.manuKyoseiZeccho > 0))
                        {
                            ...
                            var dy = Mathf.SmoothStep(0, Mathf.Clamp01(paramX), UnityEngine.Random.Range(0f, 1f)) *
                                     UnityEngine.Random.Range(0, 30f);
                            //var dy = Mathf.SmoothStep(0, Mathf.Clamp01(paramY), UnityEngine.Random.Range(0f, 1f)) * UnityEngine.Random.Range(0, 90f);
                            var dx = dy / 2;

                            var trTh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, "Bip01 R Thigh",
                                true);
                            var rot = trTh.localRotation.eulerAngles;
                            trTh.localRotation = Quaternion.Euler(rot.x + dx, rot.y + dy, rot.z);

                            trTh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, "Bip01 L Thigh",
                                true);
                            rot = trTh.localRotation.eulerAngles;
                            trTh.localRotation = Quaternion.Euler(rot.x - dx, rot.y - dy, rot.z);
                ...
                if (cfg_.doFaceSync && !slave.boMAN && mdSlave_No > 0)
                {
                    ...
                    {
                        FaceBlend2Sync(mdSlaves[0].mem.body0.Face.morph, mdSlaves[mdSlave_No].mem.body0.Face.morph,
                            false);
                    }
                }
                ...
                if (cfg_.doStackSlave)
                {
                    ...
                    if (ComExt.IsCom3d2)
                    {
                        ...
                        if (cfg.AdjustBoneHitHeightY)
                        {
                            ...
                            Transform trFloor;
                            if ((vSceneLevel == 15 || vIsKaisouScene || bIsYotogiScene) &&
                                (mdSlave_No > 0 && mdSlaves[0].mem && !mdSlaves[0].mem.boMAN))
                            {
                    ...
                    else if (cfg.AdjustBoneHitHeightY)
                    {
                        ...
                        if (cfg_.Adjust_doHitHeightYOffset)
                        {
                            //オフセット設定有り
                            slave.body0.SetBoneHitHeightY(master.gameObject.transform.position.y +
                                                          cfg_.Adjust_HitHeightYOffset);
                            //debugPrintConsole("slave.body0.BoneHitHeightY:" + slave.body0.BoneHitHeightY + " / (master.gameObject.transform.position.y:" + master.gameObject.transform.position.y + ")");
                        }
                    }

                    // 角度オフセットを位置位置オフセット前に持ってきてみる ver0025
                    if (cfg_.doStackSlave_PosSyncMode && cfg_.doStackSlave_PosSyncModeSp &&
                        !string.IsNullOrEmpty(cfg_.doStackSlave_PosSyncModeSp_TgtBone))
                    {
                        //if (cfg_.doStackSlave_PosSyncModeSp && !string.IsNullOrEmpty(cfg_.doStackSlave_PosSyncModeSp_TgtBone))
                        {
                            ...
                            if (!bnames.Contains(cfg_
                                    .doStackSlave_PosSyncModeSp_TgtBone)) // TODO just check if it exists in the armature to support custom bones
                            {
                        ...
                        if (cfg_.doStackSlave_PosSyncModeSp)
                        {
                            //角度オフセット
                            Transform slvTr;
                            slvTr = BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform,
                                (cfg_.doStackSlave_PosSyncModeSp_TgtBone), true);
                    ...
                    else if (cfg_.doStackSlave_PosSyncMode && (cfg_.doStackSlave_CliCnk || cfg_.doStackSlave_Pelvis))
                    {
                        ...
                        Transform slvTr;
                        slvTr = BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform,
                            (!master.boMAN ? "Bip01" : "ManBip"), true);
                    ...
                    else if ( /*!cfg_.doStackSlave_PosSyncMode && */
                             (cfg_.doStackSlave_CliCnk || cfg_.doStackSlave_Pelvis))
                    {
                        slave.gameObject.transform.localRotation = master.gameObject.transform.localRotation;

                        //角度オフセット
                        Transform slvTr = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                            (!slave.boMAN ? "Bip01" : "ManBip"), true);
...
#else
						var q =
 Quaternion.Inverse(slave.transform.rotation) * slvTr.rotation * Quaternion.Euler(0, -90, -90);
						var q1 = q * Quaternion.Euler(v3ofs[num_].v3StackOffsetRot);
                    ...
                    {
                        slave.gameObject.transform.localRotation = master.gameObject.transform.localRotation *
                                                                   Quaternion.Euler(v3ofs[num_].v3StackOffsetRot);
                    }
					...
					if (cfg_.doStackSlave_CliCnk)
					{
						...
						Transform slvTr =
 BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, (!slave.boMAN ? "Bip01" : "ManBip"), true);
						slave.gameObject.transform.rotation *=
 (Quaternion.Euler(slvTr.TransformDirection(Quaternion.Euler(0, -90, -90) * v3ofs[num_].v3StackOffsetRot)));
#if DEBUG
 ...
 (Quaternion.Euler(slvTr.TransformDirection(Quaternion.Euler(0, -90, -90) * v3ofs[num_].v3StackOffsetRot)));
					...
					if (cfg_.doStackSlave_PosSyncMode && (cfg_.doStackSlave_CliCnk || cfg_.doStackSlave_Pelvis))
					{
						...
						Transform slvTr;
						slvTr =
 BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, (!master.boMAN ? "Bip01" : "ManBip"), true);
						//var q = Quaternion.Inverse(master.gameObject.transform.rotation) * slvTr.rotation;
						//slave.gameObject.transform.rotation *= Quaternion.Euler(q * (Quaternion.Euler(0, -90, -90) * v3ofs[num_].v3StackOffsetRot));
...
//#endif
					{
						slave.gameObject.transform.localRotation =
 master.gameObject.transform.localRotation * Quaternion.Euler(v3ofs[num_].v3StackOffsetRot);
					}
#endif
                    // IK初期化
                    if (IkXT.IsNewIK)
                    ...
                    if (doMasterSlave && cfg_.doCopyIKTarget)
                    {
                        ...
                        {
                            if (string.IsNullOrEmpty(slave.body0._ikp().tgtHandR_AttachName) &&
                                slave.body0._ikp().tgtHandR == null)
                            {
                                if (!cfg_.chkIkSpCustomR_v2())
                                    AtccHand2HandR2(master, slave, v3ofs[num_].v3HandROffset,
                                        v3ofs[num_].v3HandROffsetRot, cfg_);
                            }

                            if (string.IsNullOrEmpty(slave.body0._ikp().tgtHandL_AttachName) &&
                                slave.body0._ikp().tgtHandL == null)
                            {
                                if (!cfg_.chkIkSpCustomL_v2())
                                    AtccHand2HandL2(master, slave, v3ofs[num_].v3HandLOffset,
                                        v3ofs[num_].v3HandLOffsetRot, cfg_);
                            }
                        }
                    }
                    ...
                    {
                        if (!cfg_.chkIkSpCustomL_v2())
                            AtccHand2HandL2(master, slave, v3ofs[num_].v3HandLOffset, v3ofs[num_].v3HandLOffsetRot,
                                cfg_);
                        if (!cfg_.chkIkSpCustomR_v2())
                            AtccHand2HandR2(master, slave, v3ofs[num_].v3HandROffset, v3ofs[num_].v3HandROffsetRot,
                                cfg_);
                    }
                    else
                    {
        ...
        static MsLinks[] _MSlinks = new MsLinks[MAX_PAGENUM]
            { new MsLinks(), new MsLinks(), new MsLinks(), new MsLinks(), new MsLinks() };
        ...
        void showSlidersHint()
        {
            if (cfg.DlgShow_Hint001)
            {
                ...
                var ret = NUty.WinMessageBox(NUty.GetWindowHandle(), msg, "( i )",
                    NUty.MSGBOX.MB_YESNO | NUty.MSGBOX.MB_ICONINFORMATION);
        ...
        {
            NUty.WinMessageBox(NUty.GetWindowHandle(), msg, "( i )",
                NUty.MSGBOX.MB_OK | NUty.MSGBOX.MB_ICONINFORMATION);
        }
        ...
        void WindowCallback_proc(int id)
        {
            ...
            EditScroll_cfg = GUI.BeginScrollView(new Rect(0, 20, GUI_WIDTH, GUI_HIGHT - 30), EditScroll_cfg,
                new Rect(0, 0, GUI_WIDTH - 16, EditScroll_cfg_sizeY));
            try
            {
                ...
                {
                    GUI.enabled = false;
                    GUI.Label(new Rect(ItemX, pos_y += ItemHeight, ItemWidth, ItemHeight), "夜伽Slave-Keeper 作動中",
                        gsButton);
                    GUI.enabled = true;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        if (GUI.Button(new Rect(ItemX, pos_y += ItemHeight, ItemWidth / 2, ItemHeight), "Man⇒Maid",
                                gsButton))
                        {
                            // モード変更
                        ...
                        if (GUI.Button(new Rect(ItemX + ItemWidth / 2, pos_y, ItemWidth / 2, ItemHeight), "Maid⇒Man",
                                gsButton))
                        {
                            // モード変更
                ...
                for (int i = 0; i < MAX_PAGENUM; i++)
                {
                    ...
                    if (GUI.Button(new Rect(ItemX + ItemWidth - (5 + 20 * (MAX_PAGENUM - i)), pos_y, 20, 20),
                            (i + 1).ToString(), gsButton))
                    {
                        _pageNum = i;
                    }
                ...
                */
                ...
                if (ms_.maidKeepSlaveYotogi)
                {
                    ...
                    rcItem.y += ItemHeight;
                    GUI.Label(rcItem,
                        (!ms_.do_master || !ms_.do_master.Visible ? " [待機中]" : " [⇔]") + "  " + ms_.do_masterName,
                        gsText);
                    rcItem.y += ItemHeight;
                    GUI.Label(rcItem, "【Slave】 (リンク先)", gsLabel);
                    rcItem.y += ItemHeight;
                    GUI.Label(rcItem,
                        (!ms_.do_slave || !ms_.do_slave.Visible ? " [待機中]" : " [⇔]") + "  " + ms_.do_slaveName, gsText);
                    rcItem.y += ItemHeight;
                ...
                else
                {
                    ...
                    if (ms_.mdMasters.Count > 0)
                    {
                        ...
                        if (ms_.mdMaster_No >= 0)
                            master_name = GetMaidName(ms_.mdMasters[ms_.mdMaster_No], true) +
                                          getMSState(ms_.mdMasters[ms_.mdMaster_No].mem);
                        ...
                        if (ComboMaster.Show(rcItem, ItemHeight, ItemHeight * 4, m_names, master_name, gsButton,
                                gsButton))
                        {
                    ...
                    if (ms_.mdSlave_No >= 0)
                        slave_name = GetMaidName(ms_.mdSlaves[ms_.mdSlave_No], true) +
                                     getMSState(ms_.mdSlaves[ms_.mdSlave_No].mem);
                    m_names = new List<string>();
                    ...
                    // Slave選択
                    if (ComboSlave.Show(rcItem, ItemHeight, ItemHeight * 4, m_names, slave_name, gsButton, gsButton,
                            disables))
                    {
                ...
                Maid master = (ms_.mdMasters.Count > 0 && !SlaveRusubanMode)
                    ? ms_.mdMasters[ms_.mdMaster_No].mem
                    : null;
                ...
                if (master != null)
                {
                    ...
                    else if (GUI.Button(rcItem,
                                 "Master-Slave *" + (_pageNum + 1) + " リンク" + (!ms_.doMasterSlave ? "実行" : "停止"),
                                 gsButton))
                    {
                        // リンクスタートか停止
                    ...
                    if (p_mscfg.doStackSlave)
                    {
                        ...
                        _Gizmo.Visible = GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - 70, ItemHeight), _Gizmo.Visible,
                            "Slave重ね位置調整(Gizmo)", gsToggle);
                        GUI.enabled = true;
                        ...
                        if (GUI.Button(new Rect(LX + ItemWidth - 70 + 40, (pos_y), 20, 20), (showPosSlider ? "-" : "+"),
                                gsButton))
                        {
                        ...
                        if (showPosSlider)
                        {
                            ...
                            GUI.Label(new Rect(LX, (pos_y += ItemHeight), 122, ItemHeight),
                                " +X: " + Math.Round(_v.x, 4), gsLabel);
                            ...
                            GUI.Label(new Rect(LX, (pos_y += ItemHeight - 5), 122, ItemHeight),
                                " +Y: " + Math.Round(_v.y, 4), gsLabel);
                        ...
                        if (showPosSlider)
                        {
                            ...
                            rcItem.y = (pos_y += 20);
                            p_mscfg.doStackSlave_Pelvis = GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - LX, ItemHeight),
                                p_mscfg.doStackSlave_Pelvis, "骨盤ボーンでの位置補正を行う", gsToggle);
                            if (p_mscfg.doStackSlave_Pelvis) p_mscfg.doStackSlave_CliCnk = false;

                            rcItem.y = (pos_y += 20);
                            p_mscfg.doStackSlave_CliCnk = GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - LX, ItemHeight),
                                p_mscfg.doStackSlave_CliCnk, "局部で位置補正(chinkoCenter)", gsToggle);
                            ...
                            rcItem.y = (pos_y += 20);
                            p_mscfg.Adjust_doHitHeightYOffset = GUI.Toggle(
                                new Rect(LX, (pos_y), ItemWidth - LX, ItemHeight), p_mscfg.Adjust_doHitHeightYOffset,
                                "Slave HitHeight調整(接地判定高さ)", gsToggle);
                            GUI.enabled = true;
                            if (p_mscfg.Adjust_doHitHeightYOffset)
                            {
                                ...
                                GUI.Label(new Rect(LX, (pos_y), LW, ItemHeight), "+Y Offset: " + Math.Round(refsc, 3),
                                    gsLabel);
                                ...
                                newsc_h = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, newsc_h, 0.01f, gsButton,
                                    -2000f, 2000f);
                                ...
                                newsc_h = GUI.HorizontalSlider(new Rect(LX, (pos_y += 20), SLDW, 15), newsc_h,
                                    -2f + hsldPreHeight, 2f + hsldPreHeight);
                                if (newsc_h != refsc)
                                {
                                    refsc = newsc_h;
                                }
                        ...
                        rcItem.y = (pos_y += 20);
                        p_mscfg.doIKTargetMHand = GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - LX - 70, ItemHeight),
                            p_mscfg.doIKTargetMHand, "Masterの両手にアタッチ", gsToggle);
                        //pos_y = (int)(rcItem.y += ItemHeight);
                        ...
                        if (ms_.doMasterSlave)
                        {
                            ...
                            if (GUI.Button(new Rect(LX + ItemWidth - 70 + 40, (pos_y), 20, 20),
                                    (showPosSliderHand ? "-" : "+"), gsButton))
                            {
                        ...
                        rcItem.y = (pos_y += 20);
                        p_mscfg.doCopyIKTarget = GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - LX, ItemHeight),
                            p_mscfg.doCopyIKTarget, "Masterの手のIKターゲットを複製", gsToggle);

                        rcItem.y = (pos_y += 20);
                        p_mscfg.doStackSlave_PosSyncMode =
                            GUI.Toggle(new Rect(LX, (pos_y), ItemWidth - LX - 30, ItemHeight),
                                p_mscfg.doStackSlave_PosSyncMode, "位置のみリンク(ﾎﾟｰｽﾞ同期OFF)", gsToggle);
                        if (p_mscfg.doStackSlave_PosSyncMode)
                        {
                            p_mscfg.doStackSlave_PosSyncModeV2 = GUI.Toggle(
                                new Rect(LX + ItemWidth - LX - 30, (pos_y), 40, ItemHeight),
                                p_mscfg.doStackSlave_PosSyncModeV2, "v2", gsToggle);

                            rcItem.y = (pos_y += 20);
                            p_mscfg.doStackSlave_PosSyncModeSp = GUI.Toggle(
                                new Rect(LX, (pos_y), ItemWidth - LX - 30, ItemHeight),
                                p_mscfg.doStackSlave_PosSyncModeSp, "リンク先ボーン指定変更", gsToggle);
                            if (p_mscfg.doStackSlave_PosSyncModeSp)
                            {
                                ...
                                if (ComboPosLinkBone.Show(rctmp, ItemHeight, ItemHeight * 4, e_names,
                                        p_mscfg.doStackSlave_PosSyncModeSp_TgtBone, gsButton, gsButton))
                                {
                ...
                if (master != null)
                {
                    sc_m = (master.gameObject.transform.localScale.x + master.gameObject.transform.localScale.y +
                            master.gameObject.transform.localScale.z) / 3;
                    newsc_m = sc_m;

                    rcItem.y = (pos_y += 15);
                    GUI.Label(new Rect(SLX, (pos_y), SLW, ItemHeight), "Masterサイズ調整: " + Math.Round(newsc_m, 3),
                        gsLabel);
                    newsc_m = btnset_LR(new Rect(SLX + SLW, (pos_y), SBW, 20), SBW, newsc_m,
                        gsButton /*, cfg.Scale_Min, cfg.Scale_Max*/, 0.01f, 100f);
                    if (GUI.Button(new Rect(SLX + SEDTW, (pos_y), 28, 20), "|", gsButton))
                    {
                        newsc_m = 1f;
                    }

                    newsc_m = GUI.HorizontalSlider(new Rect(SLX, (pos_y += 20), SSLDW, 15), newsc_m, cfg.Scale_Min,
                        cfg.Scale_Max);
                    if (newsc_m != sc_m)
                    {
                ...
                float sc = (slave.gameObject.transform.localScale.x + slave.gameObject.transform.localScale.y +
                            slave.gameObject.transform.localScale.z) / 3;
                float newsc = sc;
                rcItem.y = (pos_y += 15);
                //GUI.Label(rcItem, "Slaveサイズ調整", gsLabel);
                GUI.Label(new Rect(SLX, (pos_y), SLW, ItemHeight), "Slaveサイズ調整: " + Math.Round(newsc, 3), gsLabel);
                newsc = btnset_LR(new Rect(SLX + SLW, (pos_y), SBW, 20), SBW, newsc,
                    gsButton /*, cfg.Scale_Min, cfg.Scale_Max*/, 0.01f, 100f);
                if (GUI.Button(new Rect(SLX + SEDTW, (pos_y), 28, 20), "|", gsButton))
                {
                    newsc = 1f;
                ...
                newsc = GUI.HorizontalSlider(new Rect(SLX, (pos_y += 20), SSLDW, 15), newsc, cfg.Scale_Min,
                    cfg.Scale_Max);
                if (newsc != sc)
                {
                ...
                if (!p_mscfg.Scale_HitCheckDetail)
                    GUI.Label(new Rect(SLX, (pos_y), SLW - 45, ItemHeight),
                        "HitCheck倍率: " + Math.Round(p_mscfg.Scale_HitCheckEffect, 2), gsLabel);
                else
                ...
                {
                    if (GUI.Button(new Rect(SLX + SLW, (pos_y), 50, ItemHeight),
                            (cfg.hideHitScaleDef ? "+ 開く" : "- 隠す"), gsButton))
                    {
                    ...
                    if (!cfg.hideHitScaleDef)
                    {
                        ...
                        foreach (var h in HitCheckTgtStr)
                        {
                            ...
                            GUI.Label(new Rect(SLX, (pos_y), SLW - 45, ItemHeight),
                                fixHitNames(h.Key.ToString()) + ": " + Math.Round(refsc, 3), gsLabel);
                            newsc_h = btnset_LR(new Rect(SLX + SLW, (pos_y), SBW, 20), SBW, newsc_h, 0.01f, gsButton,
                                0f, 6.0f);
                            if (GUI.Button(new Rect(SLX + SEDTW, (pos_y), 28, 20), "|", gsButton))
                            {
                                if (cfg.doHitScaleDef)
                        ...
                        if (p_mscfg.Scale_HitCheckDetail)
                        {
                            ...
                            if (GUI.Button(new Rect(SLX, (pos_y), SSLDW - 80, 20),
                                    "全ﾒｲﾄﾞの基本値に" + (cfg.doHitScaleDef ? "上書き" : "登録"), gsButton))
                            {
                ...
                p_mscfg.Scale_HitCheckDetail = GUI.Toggle(new Rect(SLX, (pos_y), ItemWidth - 120, ItemHeight),
                    p_mscfg.Scale_HitCheckDetail, "HitCheck詳細設定", gsToggle);
                if (cfg.doHitScaleDef)
                {
                    Color cbk = GUI.color;
                    GUI.color = Color.yellow;
                ...
                if (master != null)
                {
                    ...
                    FieldInfo fiml = typeof(TBodySkin).GetField("m_listManAlphaMat",
                        BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
                    if (fiml != null)
                    {
                        ...
                        GUI.Label(rcItem, (master.boMAN ? "Master" : "Slave") + "の透明度: " + Math.Round(manAlpha, 1),
                            gsLabel);

                        Color cbk = GUI.color;
                        if (holdAlphaBk)
                            GUI.color = Color.yellow;
                        bool holdAlpha = GUI.Toggle(new Rect(SLX + SEDTW - 60, (pos_y), 60, ItemHeight), holdAlphaBk,
                            "Hold", gsToggle);
                        ...
                        int newA = (int)GUI.HorizontalSlider(new Rect(rcItem.x, rcItem.y, rcItem.width, 15), manAlpha,
                            0, 100);
                        if (newA != manAlpha)
                        {
                ...
                if (master != null)
                {
                    ...
                    //if (master.boMAN) m/s共通に
                    {
                        ...
                        if (master.boMAN)
                        {
                            ...
                            bocnk = GUI.Toggle(new Rect(rcItem.x, rcItem.y, rcItem.width - 80, rcItem.height), bochk,
                                "Masterの局部を表示", gsToggle);
                            pos_y = (int)(rcItem.y += ItemHeight);
                            GUI.color = cbk;
                        }
                        else if (slave.boMAN)
                        ...
                        if (tgtman && /*master.body0.trManChinko != null && */(bocnk || showChinkoSlider))
                        {
                            ...
                            if (showChinkoSlider)
                            {
                                ...
                                float _s = s;
                                GUI.Label(new Rect(LX, (pos_y), 122, ItemHeight), " サイズ: " + Math.Round(_s, 3),
                                    gsLabel);
                                ...
                                Vector3 v0 = _v;
                                {
                                    ...
                                    _v.x = GUI.HorizontalSlider(new Rect(LX, (pos_y += 20), SLDW, 15), _v.x, -0.15f,
                                        0.15f);
                                    ...
                                    _v.y = GUI.HorizontalSlider(new Rect(LX, (pos_y += 20), SLDW, 15), _v.y, -0.15f,
                                        0.15f);
                                    ...
                                    _v.z = GUI.HorizontalSlider(new Rect(LX, (pos_y += 20), SLDW, 15), _v.z, -0.15f,
                                        0.15f);
                ...
                if (!slave.boMAN)
                {
                    ...
                    bool holdSlvMask_t = GUI.Toggle(new Rect(ItemX + 20 + (ItemWidth - 20 - 90), pos_y, 90, ItemHeight),
                        ms_.holdSlvMask, "マスクを保持", gsToggle);
                    GUI.color = cbklt;
                ...
                if (master != null)
                {
                    ...
                    if (!slave.boMAN)
                    {
                        ...
                        p_mscfg.doVoiceAndFacePlay =
                            GUI.Toggle(rctmp0, p_mscfg.doVoiceAndFacePlay, "Slaveサブメイド夜伽演出", gsToggle);
                        pos_y = (int)(rcItem.y += ItemHeight);
                        if (p_mscfg.doVoiceAndFacePlay)
                        {
                            ...
                            if (GUI.Button(new Rect(ItemX + 10, (pos_y), 20, 20), (showVymPlaySet ? "-" : "+"),
                                    gsButton))
                            {
                                showVymPlaySet = !showVymPlaySet;
                            }

                            GUI.Label(new Rect(ItemX + 20 + 10, pos_y, ItemWidth - 20 - 10, ItemHeight), "夜伽演出オプション",
                                gsLabel);
                            pos_y = (int)(rcItem.y += ItemHeight);
                            ...
                            if (showVymPlaySet)
                            {
                                ...
                                VYMModule.VymModule.cfg.HohoEnabled = GUI.Toggle(rctmp,
                                    VYMModule.VymModule.cfg.HohoEnabled, "頬染め", gsToggle);
                                rctmp.x += rctmp.width;
                                VYMModule.VymModule.cfg.YodareEnabled = GUI.Toggle(rctmp,
                                    VYMModule.VymModule.cfg.YodareEnabled, "ヨダレ", gsToggle);
                                rctmp.x += rctmp.width;
                                VYMModule.VymModule.cfg.NamidaEnabled = GUI.Toggle(rctmp,
                                    VYMModule.VymModule.cfg.NamidaEnabled, "ナミダ", gsToggle);
                                pos_y = (int)(rcItem.y += ItemHeight);

                                GUI.Label(new Rect(ItemX + 10, pos_y, ItemWidth - 20, ItemHeight), "【ボイスモード選択】",
                                    gsLabel);
                                pos_y = (int)(rcItem.y += ItemHeight);
                                ...
                                if (ComboVoiceMode.Show(rctmp, ItemHeight, ItemHeight * 4, e_names, null, gsButton,
                                        gsButton))
                                {
                                    if (ComboVoiceMode.sIndex >= 0)
                                    {
                                        VYMModule.VymModule.cfg.eVoiceMode =
                                            (VYMModule.VymModule.VoiceMode)ComboVoiceMode.sIndex;
                                    }
                                }
                                ...
                                bool manu_e = GUI.Toggle(new Rect(ItemX + 10, pos_y, ItemWidth - 20, ItemHeight),
                                    p_mscfg.doManualVfPlay,
                                ...
                                if (p_mscfg.doManualVfPlay)
                                {
                                    ...
                                    {
                                        //拡張Lvスタン用
                                        GUI.Label(new Rect(MvpX, pos_y, ItemWidth, ItemHeight), "興奮: - スタン状態 -",
                                            gsLabel);
                                    }
                                    else
                                    {
                                        ...
                                        GUI.Label(new Rect(MvpX + tsize.x, pos_y, ItemDw * 3, ItemHeight),
                                            "" + p_mscfg.manuVf_iExcite, gsLabel);

                                        p_mscfg.manuVf_iExcite = (int)GUI.HorizontalSlider(
                                            new Rect(ItemX + 10 + ItemDw * 4 - 10, pos_y + 5,
                                                ItemWidth - ItemDw * 4 - 10 - 45, ItemHeight - 5),
                                            p_mscfg.manuVf_iExcite, 0, 300);
                                        GUI.color = cbk_mn;
                                    }

                                    if (GUI.Toggle(
                                            new Rect(ItemX + 10 + ItemDw * 4 + (ItemWidth - ItemDw * 4 - 10 - 45),
                                                pos_y, 45, ItemHeight), (p_mscfg.manuVf_iExcite >= 1000), "Stun",
                                            gsToggle) /* && (p_mscfg.manuVf_iExcite < 1000)*/)
                                    ...
                                    if (GUI.Toggle(rctgl, (p_mscfg.manuVf_mState == 10), "止", gsToggle) &&
                                        (p_mscfg.manuVf_mState != 10))
                                        p_mscfg.manuVf_mState = 10;
                                    rctgl.x += rctgl.width;
                                    if (GUI.Toggle(rctgl, (p_mscfg.manuVf_mState == 20), "弱", gsToggle) &&
                                        (p_mscfg.manuVf_mState != 20))
                                        p_mscfg.manuVf_mState = 20;
                                    rctgl.x += rctgl.width;
                                    if (GUI.Toggle(rctgl, (p_mscfg.manuVf_mState == 30), "強", gsToggle) &&
                                        (p_mscfg.manuVf_mState != 30))
                                        p_mscfg.manuVf_mState = 30;
                                    rctgl.x += rctgl.width;
                                    if (GUI.Toggle(rctgl, (p_mscfg.manuVf_mState == 40), "余韻", gsToggle) &&
                                    ...
                                    if (GUI.Button(new Rect(MvpX + 5, pos_y, ItemWidth - 30, ItemHeight), "強制絶頂の実行"
                                            + (ms_.manuKyoseiZeccho <= 0 ? "" : " +" + ms_.manuKyoseiZeccho.ToString()),
                                            gsButton))
                                    {
                                    ...
                                    GUI.Label(new Rect(MvpX, pos_y, ItemWidth - 20, ItemHeight * 3),
                                        "※表情、ボイスは設定変更時やMasterのモーション変化時にini設定テーブルからランダム選択", gsLabel);
                                ...
                                bool mz = GUI.Toggle(new Rect(ItemX + 10, pos_y, ItemDw * 6, ItemHeight),
                                    p_mscfg.manuVf_mOrgcmb >= 0, "絶頂数ロック: " + mp.vOrgasmCmb.ToString(), gsToggle);
                                if (!mz)
                                {
                                    GUI.enabled = false;
                                    GUI.HorizontalSlider(
                                        new Rect(ItemX + 10 + ItemDw * 6, pos_y + 5, ItemWidth - ItemDw * 6 - 10,
                                            ItemHeight - 5), mp.vOrgasmCmb, 0, 5);
                                ...
                                else
                                {
                                    ...
                                    p_mscfg.manuVf_mOrgcmb = (int)GUI.HorizontalSlider(
                                        new Rect(ItemX + 10 + ItemDw * 6, pos_y + 5, ItemWidth - ItemDw * 6 - 10,
                                            ItemHeight - 5), p_mscfg.manuVf_mOrgcmb, 0, 5);
                                }

                                pos_y = (int)(rcItem.y += ItemHeight);

                                p_mscfg.doZecchoKeiren = GUI.Toggle(new Rect(ItemX + 10, pos_y, ItemDw * 6, ItemHeight),
                                    p_mscfg.doZecchoKeiren, "絶頂痙攣 β: " + Math.Round(p_mscfg.fZecchoKeirenParam, 3),
                                    gsToggle);
                                if (p_mscfg.doZecchoKeiren)
                                    p_mscfg.fZecchoKeirenParam = GUI.HorizontalSlider(
                                        new Rect(ItemX + 10 + ItemDw * 6, pos_y + 5, ItemWidth - ItemDw * 6 - 10,
                                            ItemHeight - 5), p_mscfg.fZecchoKeirenParam, 0, 0.5f);
                                pos_y = (int)(rcItem.y += ItemHeight);
                            }
                        }
                        ...
                        if (bIsVymPlg && p_mscfg.doVoiceAndFacePlay)
                        {
                            ...
                            rcV.x += rcV.width;
                            rcV.width = 56;
                            p_mscfg.doVoiceAndFacePlayOnVYM_Zeccho = GUI.Toggle(rcV,
                                p_mscfg.doVoiceAndFacePlayOnVYM_Zeccho, "⇔絶頂", gsToggle);
                ...
                if (!slave.boMAN)
                {
                    ...
                    if (showSlaveEyeToTgt)
                    {
                        ...
                        if (!slave.boMAN && ms_.mdSlave_No > 0 && ms_.mdSlaves[0].mem)
                        {
                            ...
                            Transform tgt_tr =
                                BoneLink.BoneLink.SearchObjName(tgt.body0.m_Bones.transform, tgtBoneName, true);
                ...
                if (p_mscfg.doIKTargetMHandSpCustom)
                {
                    p_mscfg.doIKTargetMHandSpCustom_v2 =
                        GUI.Toggle(new Rect(ItemX + (ItemWidth - 60), pos_y, 60, ItemHeight),
                            p_mscfg.doIKTargetMHandSpCustom_v2,
                            "v2", gsToggle);
                }
                ...
                else
                {
                    ...
                    if (GUI.Button(new Rect(ItemX + 100, (pos_y), ItemWidth - 100, 20),
                            p_mscfg.doIKTargetMHandSpR_TgtChar.ToString(), gsButton))
                    {
                        change_tgt(true, ref p_mscfg.doIKTargetMHandSpR_TgtChar, +1);
                    }
                    ...
                    if (GUI.Button(new Rect(ItemX + 100, (pos_y), ItemWidth - 100, 20),
                            p_mscfg.doIKTargetMHandSpL_TgtChar.ToString(), gsButton))
                    {
                        change_tgt(false, ref p_mscfg.doIKTargetMHandSpL_TgtChar, +1);
                    }
                    ...
                    void seth(bool boR)
                    {
                        ...
                        {
                            if (GUI.Button(new Rect(LX + (ItemWidth - 20 - 40), pos_y, 40, ItemHeight),
                                    (open ? "閉" : "調整"), gsButton))
                            {
                            ...
                            //if (showHandTTPosSlider)
                            {
                                ...
                                if (boR)
                                {
                                    ...
                                    p_mscfg.doIKTargetMHandSpCustomAltRotR =
                                        GUI.Toggle(new Rect(ItemX + 10, pos_y, ItemWidth, ItemHeight),
                                            p_mscfg.doIKTargetMHandSpCustomAltRotR,
								...
								if (changeR && ms_.doMasterSlave && InputEx.GetModifierKey(InputEx.ModifierKey.Alt))
								{
									...
									//Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnR(slave), true);
									Transform trh =
 BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, GetHandBnR(master), true);
									var iks = slave.body0._ikp();
									...
									if (tgtMaid != null && tgtMaid.body0 != null && tgtMaid.body0.goSlot[iks.tgtHandR_AttachSlot].morph != null)
									{
										...
										// アタッチポイント基準角度
										if (p_mscfg.doIKTargetMHandSpCustomAltRotR)
											p_v3of.v3HandROffsetRot =
 (Quaternion.Inverse(rotation2) * trh.rotation * Quaternion.Euler(p_v3of.v3HandROffsetRot)).eulerAngles;
										else
											p_v3of.v3HandROffsetRot =
 (Quaternion.Inverse(trh.rotation) * rotation2 * Quaternion.Euler(p_v3of.v3HandROffsetRot)).eulerAngles;
									}
								}
								if (changeL && ms_.doMasterSlave && InputEx.GetModifierKey(InputEx.ModifierKey.Alt))
								{
									...
									//Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnL(slave), true);
									Transform trh =
 BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, GetHandBnL(master), true);
									var iks = slave.body0._ikp();
									...
									if (tgtMaid != null && tgtMaid.body0 != null && tgtMaid.body0.goSlot[iks.tgtHandL_AttachSlot].morph != null)
									{
										...
										// アタッチポイント基準角度
										if (p_mscfg.doIKTargetMHandSpCustomAltRotL)
											p_v3of.v3HandLOffsetRot =
 (Quaternion.Inverse(rotation2) * trh.rotation * Quaternion.Euler(p_v3of.v3HandLOffsetRot)).eulerAngles;
										else
											p_v3of.v3HandLOffsetRot =
 (Quaternion.Inverse(trh.rotation) * rotation2 * Quaternion.Euler(p_v3of.v3HandLOffsetRot)).eulerAngles;
									}
								}
#endif
                            }
                ...
                if (IkXT.IsNewIK)
                {
                    ...
                    if (IkXT.IsIkCtrlO118)
                    {
                        ...
                        p_mscfg.doFinalIKShoulderMove =
                            GUI.Toggle(new Rect(ItemX + (ItemWidth - 95), pos_y, 30, ItemHeight),
                                p_mscfg.doFinalIKShoulderMove,
                                "肩", gsToggle);

                        p_mscfg.doFinalIKThighMove =
                            GUI.Toggle(new Rect(ItemX + (ItemWidth - 65), pos_y, 30, ItemHeight),
                                p_mscfg.doFinalIKThighMove,
                                "腿", gsToggle);

                        p_mscfg.fFinalIKLegWeight =
                            GUI.Toggle(new Rect(ItemX + (ItemWidth - 35), pos_y, 30, ItemHeight),
                                p_mscfg.fFinalIKLegWeight > 0f,
                                "足", gsToggle)
                                ? 1.0f
                                : 0.0f;
                ...
                if (p_mscfg.doBlendHandR)
                {
                    ...
                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight), "適用: " + Math.Round(p_mscfg.fBlendHandR, 2),
                        gsLabel);
                    p_mscfg.fBlendHandR = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandR, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);

                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight),
                        "開度: " + Math.Round(p_mscfg.fBlendHandROpen, 2), gsLabel);
                    p_mscfg.fBlendHandROpen = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandROpen, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);

                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight),
                        "握り: " + Math.Round(p_mscfg.fBlendHandRGrip, 2), gsLabel);
                    p_mscfg.fBlendHandRGrip = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandRGrip, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);
                    ...
                    {
                        {
                            ...
                            GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight), "速さ: " + Math.Round(val, 2),
                                gsLabel);
...

                        {
                            ...
                            GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight), "握力: " + Math.Round(val, 2),
                                gsLabel);
                ...
                if (p_mscfg.doBlendHandL)
                {
                    const int SLDS = ItemX + 10 + 90 + 5;
                    const int SLDW = 220 - 10 - 90;
                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight), "適用: " + Math.Round(p_mscfg.fBlendHandL, 2),
                        gsLabel);
                    p_mscfg.fBlendHandL = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandL, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);

                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight),
                        "開度: " + Math.Round(p_mscfg.fBlendHandLOpen, 2), gsLabel);
                    p_mscfg.fBlendHandLOpen = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandLOpen, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);

                    GUI.Label(new Rect(ItemX + 10, pos_y, 90, ItemHeight),
                        "握り: " + Math.Round(p_mscfg.fBlendHandLGrip, 2), gsLabel);
                    p_mscfg.fBlendHandLGrip = GUI.HorizontalSlider(new Rect(SLDS, (pos_y), SLDW, ItemHeight),
                        p_mscfg.fBlendHandLGrip, 0f, 1f);
                    pos_y = (int)(rcItem.y += ItemHeight);
        ...
        static Maid GetHandAtcTgt(bool boR, Maid slave, Maid master, MsLinkConfig p_mscfg)
        {
            ...
            var chk = boR
                ? p_mscfg.doIKTargetMHandSpR_TgtChar
        ...
        // 手のアタッチ調整ギズモ＆スライダー
        private bool ProcHandGizmo(Maid slave, ref Rect rcItem, ref int pos_y, MsLinkConfig p_mscfg,
            v3Offsets p_v3of_org, bool boSelectLR = true)
        {
            bool bochg = false;
            v3OffsetsV2 p_v3of2 =
                boSelectLR ? new v3OffsetsV2(p_v3of_org, false) : new v3OffsetsV2(p_v3of_org, p_mscfg); // v5.0
            ...
            if (slave)
            {
                ...
                if (p_mscfg.doIKTargetMHandSpCustom && p_mscfg.doIKTargetMHandSpCustom_v2)
                {
                    ...
                    if (p_mscfg.doIKTargetMHandSpCustomAltRotR && p_mscfg.doIKTargetMHandSpR_TgtChar != ATgtChar.None)
                    {
                        // アタッチ先座標
                        var iks = slave.body0._ikp();
                        if (iks != null && iks.tgtMaidR != null && iks.tgtHandR_AttachSlot >= 0 &&
                            !string.IsNullOrEmpty(iks.tgtHandR_AttachName))
                        {
                            if (iks.tgtMaidR.body0 != null &&
                                iks.tgtMaidR.body0.goSlot[iks.tgtHandR_AttachSlot].morph != null)
                            {
                                Vector3 vector2;
                                Quaternion rotation2;
                                ...
                                // アタッチポイント基準角度
                                Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                                    (!slave.boMAN ? "Bip01" : "ManBip") + " R Hand", true);
                                //Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnR(slave), true);
                    ...
                    if (p_mscfg.doIKTargetMHandSpCustomAltRotL && p_mscfg.doIKTargetMHandSpL_TgtChar != ATgtChar.None)
                    {
                        // アタッチ先座標
                        var iks = slave.body0._ikp();
                        if (iks != null && iks.tgtMaidL != null && iks.tgtHandL_AttachSlot >= 0 &&
                            !string.IsNullOrEmpty(iks.tgtHandL_AttachName))
                        {
                            if (iks.tgtMaidL.body0 != null &&
                                iks.tgtMaidL.body0.goSlot[iks.tgtHandL_AttachSlot].morph != null)
                            {
                                //Console.WriteLine("g-l");
                                Vector3 vector2;
                                Quaternion rotation2;
                                iks.tgtMaidR.body0.goSlot[iks.tgtHandL_AttachSlot].morph
                                    .GetAttachPoint(iks.tgtHandL_AttachName, out vector2, out rotation2);

                                // アタッチポイント基準角度
                                Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                                    (!slave.boMAN ? "Bip01" : "ManBip") + " L Hand", true);
                                //Transform trh = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnL(slave), true);
                ...
                if (showPosSliderHandR && _Gizmo_HandR.isDrag)
                {
                    ...
                    Transform slvR = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                        (!slave.boMAN ? "Bip01" : "ManBip") + " R Hand", true);
                    //座標変換して加算
                    p_v3of2.v3HandROffset +=
                        Quaternion.Euler(p_v3of2.v3HandROffsetRot) * slvR.InverseTransformDirection(dv);
                ...
                if (!showPosSliderHandR && _Gizmo_HandL.isDrag)
                {
                    ...
                    Transform slvL = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                        (!slave.boMAN ? "Bip01" : "ManBip") + " L Hand", true);

                    //座標変換して加算
                    p_v3of2.v3HandLOffset +=
                        Quaternion.Euler(p_v3of2.v3HandLOffsetRot) * slvL.InverseTransformDirection(dv);
                ...
                // 位置
                {
                    ...
                    GUI.Label(new Rect(LX, (pos_y += ItemHeight), 122, ItemHeight), " +X: " + Math.Round(_v.x, 4),
                        gsLabel);
                    _v.x = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.x, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                    ...
                    GUI.Label(new Rect(LX, (pos_y += ItemHeight - 5), 122, ItemHeight), " +Y: " + Math.Round(_v.y, 4),
                        gsLabel);
                    _v.y = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.y, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                    ...
                    GUI.Label(new Rect(LX, (pos_y += ItemHeight - 5), 122, ItemHeight), " +Z: " + Math.Round(_v.z, 4),
                        gsLabel);
                    _v.z = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.z, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                ...
                // 回転
                {
                    Vector3 _v = showPosSliderHandR
                        ? va180(p_v3of2.v3HandROffsetRot)
                        : va180(p_v3of2.v3HandLOffsetRot);

                    GUI.Label(new Rect(LX, (pos_y += ItemHeight), 122, ItemHeight), " +回転X: " + Math.Round(_v.x, 4),
                        gsLabel);
                    _v.x = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.x, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                    ...
                    GUI.Label(new Rect(LX, (pos_y += ItemHeight - 5), 122, ItemHeight), " +回転Y: " + Math.Round(_v.y, 4),
                        gsLabel);
                    _v.y = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.y, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                    ...
                    GUI.Label(new Rect(LX, (pos_y += ItemHeight - 5), 122, ItemHeight), " +回転Z: " + Math.Round(_v.z, 4),
                        gsLabel);
                    _v.z = btnset_LR(new Rect(LX + LW, (pos_y), BW, 20), BW, _v.z, gsButton);
                    if (GUI.Button(new Rect(LX + EDTW, (pos_y), 28, 20), "CL", gsButton))
                    {
                ...
                if (!_Gizmo_HandL.isDrag)
                {
                    ...
                    BkupGizmoU1(_Gizmo_HandL, p_v3of2.v3HandLOffsetRot);
							...
							{
								Transform slvR =
 BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, (!slave.boMAN ? "Bip01" : "ManBip") + " R Hand", true);
								_Gizmo_HandR.transform.parent.transform.position = slvR.position;
								_Gizmo_HandR.transform.parent.transform.rotation =
 slvR.rotation; //* Quaternion.Euler(180, 0, 0);
							...
							{
								Transform slvL =
 BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, (!slave.boMAN ? "Bip01" : "ManBip") + " L Hand", true);
								_Gizmo_HandL.transform.parent.transform.position = slvL.position;
								_Gizmo_HandL.transform.parent.transform.rotation =
 slvL.rotation; //* Quaternion.Euler(0, 0, 180);
        ...
        private bool ProcSubMemberCtrls(ref int pos_y, ref Rect rcItem, MsLinks ms_, Maid slave, MsLinkConfig p_mscfg)
        {
            ...
            if (_StockMaids.Count > 0)
            {
                ...
                if (ComboSubMaid.Show(rcItem, ItemHeight, ItemHeight * 5, s_names.ToArray(), "サブメイドを選択", gsButton,
                        gsButton))
                {
            ...
            if (_StockMaids_Visible.Count > 0)
            {
                ...
                if (ComboSubMaidV.Show(rcItem, ItemHeight, ItemHeight * 3, s_names.ToArray(), "サブメイドを選択", gsButton,
                        gsButton))
                {
                    if (ComboSubMaidV.sIndex >= 0)
                    {
                        ...
                        if (GetMaidName(_StockMaids_Visible[ComboSubMaidV.sIndex]) == ComboSubMaidV.sSelected)
                            ...
                            Console.WriteLine("メイドが一致しないため非表示をキャンセル。選択:" +
                                              GetMaidName(_StockMaids_Visible[ComboSubMaidV.sIndex]) + "/" +
                                              ComboSubMaidV.sSelected);
            ...
            rcItem.y = (pos_y);
            {
                ...
                //if (!ms_.doMasterSlave)
                //  GUI.contentColor = Color.gray;
                p_mscfg.doKeepSlaveYotogi =
                    GUI.Toggle(rcItem, p_mscfg.doKeepSlaveYotogi, "夜伽中,サブメイドのSlaveリンク維持", gsToggle);
                //GUI.contentColor = cbk;
            }
        ...
        static void PosGizmoProc(MsLinks ms_)
        {
            if ( /*_Gizmo.Visible && */_Gizmo.isDrag)
            {
                ...
                else if (_Gizmo._backup_rot != _Gizmo.transform.rotation)
                {
                    if (Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.LeftShift))
                    {
                        _Gizmo.transform.rotation =
                            Quaternion.Slerp(_Gizmo._backup_rot, _Gizmo.rotation, GIZMO_SLOWRATE);
                    }
					...
					if (cfgs[ms_.num_].doStackSlave_CliCnk)
					{
						//Maid slave = ms_.mdSlaves[ms_.mdSlave_No].mem;
						//Transform slvTr = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, (!slave.boMAN ? "Bip01 Pelvis" : "chinkoCenter"), true);
						v3ofs[_pageNum].v3StackOffsetRot =
 (Quaternion.Inverse(_Gizmo._backup_rot_u1) * _Gizmo.transform.rotation).eulerAngles;
					}
					else if(cfgs[ms_.num_].doStackSlave_Pelvis)
					{
						//Maid slave = ms_.mdSlaves[ms_.mdSlave_No].mem;
 ...
 (Quaternion.Inverse(_Gizmo._backup_rot_u1) * _Gizmo.transform.rotation).eulerAngles;
					...
					{
						v3ofs[_pageNum].v3StackOffsetRot =
 (Quaternion.Inverse(ms_.mdMasters[ms_.mdMaster_No].mem.gameObject.transform.rotation) * _Gizmo.transform.rotation).eulerAngles;
					}
//#else
 ...
 (Quaternion.Inverse(_Gizmo._backup_rot_u1) * _Gizmo.transform.rotation).eulerAngles;
						...
						{
							v3ofs[_pageNum].v3StackOffsetRot =
 _Gizmo.transform.TransformDirection(v3ofs[_pageNum].v3StackOffsetRot);
							slvTr = slave.gameObject.transform;
							v3ofs[_pageNum].v3StackOffsetRot =
 slvTr.InverseTransformDirection(Quaternion.Euler(0, -90, -90)*v3ofs[_pageNum].v3StackOffsetRot);
						}
					}
					else
					{
 ...
 (Quaternion.Inverse(_Gizmo._backup_rot_u1) * _Gizmo.transform.rotation).eulerAngles;
                    ...
                    v3ofs[_pageNum].v3StackOffsetRot =
                        (Quaternion.Inverse(_Gizmo._backup_rotLocal_u1) * _Gizmo.transform.localRotation).eulerAngles;
                    //_Gizmo.BkupPosAndRotLocalU1();

                    v3ofs[_pageNum].v3StackOffsetRot = va180(v3ofs[_pageNum].v3StackOffsetRot);
                    debugPrintConsole("_g rot: " + _Gizmo.transform.rotation.eulerAngles + " ofs: " +
                                      v3ofs[_pageNum].v3StackOffsetRot);
                }
            ...
            else if (_Gizmo.Visible)
            {
                ...
                if (cfgs[ms_.num_].doStackSlave_CliCnk)
                {
                    sTr = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                        (!slave.boMAN ? "Bip01 Pelvis" : "chinkoCenter"), true);
                }
                else if (cfgs[ms_.num_].doStackSlave_Pelvis)
                {
                    sTr = BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform,
                        (!slave.boMAN ? "Bip01" : "ManBip") + " Pelvis", true);
                }
                ...
                if (cfgs[ms_.num_].doStackSlave_PosSyncMode && cfgs[ms_.num_].doStackSlave_PosSyncModeSp &&
                    !string.IsNullOrEmpty(cfgs[ms_.num_].doStackSlave_PosSyncModeSp_TgtBone))
                {
                    Maid master = ms_.mdMasters[ms_.mdMaster_No].mem;

                    sTr = BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform,
                        (cfgs[ms_.num_].doStackSlave_PosSyncModeSp_TgtBone), true);
                    if (sTr)
                    {
                        _Gizmo.transform.rotation = sTr.rotation;
                        //if ((cfgs[ms_.num_].doStackSlave_Pelvis || cfgs[ms_.num_].doStackSlave_CliCnk))
                ...
                else if (cfgs[ms_.num_].doStackSlave_PosSyncMode &&
                         (cfgs[ms_.num_].doStackSlave_Pelvis || cfgs[ms_.num_].doStackSlave_CliCnk))
                {
                    Maid master = ms_.mdMasters[ms_.mdMaster_No].mem;
                ...
                _Gizmo.BkupPosAndRotLocalU1();
                _Gizmo._backup_rotLocal_u1 = _Gizmo._backup_rotLocal_u1 *
                                             Quaternion.Inverse(Quaternion.Euler(v3ofs[_pageNum].v3StackOffsetRot));
            }
        }
        ...
        static bool LoadMaid(Maid newmaid)
        {
            ...
            {
                if (GameMain.Instance.CharacterMgr.GetMaid(k) == null ||
                    GameMain.Instance.CharacterMgr.GetMaid(k) == newmaid)
                {
                    break;
                }
            ...
            //アクティブメイド登録
            Console.WriteLine("アクティブメイド登録: " + GetMaidName(newmaid) + "⇒" + k + " / " +
        ...
        class EasyCombo
        {
            ...
            public bool Show(Rect rect, int itemH, int maxH, List<string> slist, string sBtn, GUIStyle gsBtn,
                GUIStyle gsLst, bool[] disables = null)
            {
                return Show(rect, itemH, maxH, slist.ToArray(), sBtn, gsBtn, gsLst, disables);
            }

            public bool Show(Rect rect, int itemH, int maxH, int Numbers, string sBtn, GUIStyle gsBtn, GUIStyle gsLst,
                bool[] disables = null)
            {
            ...
            public bool Show(Rect rect_btn, int itemH, int maxH, string[] slist, string sBtn, GUIStyle gsBtn,
                GUIStyle gsLst, bool[] disables = null)
            {
            ...
            public bool Show(Rect rect_btn, int itemH, Rect rect_list, string[] slist, string sBtn, GUIStyle gsBtn,
                GUIStyle gsLst, bool[] disables = null)
            {
                int maxlen = 0;
                ...
                if (boPop)
                {
                    ...
                    scrollPosition = GUI.BeginScrollView(rect_btn, scrollPosition,
                        new Rect(0, 0, iw, itemH * slist.Length), false, true);
                    ...
                    if (boPop && !boChanged)
                    {
                        if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonUp(0) || Input.GetMouseButtonDown(1) ||
...
*/
        ...
        */
            ...
            if (!boEnabled)
            {
                ...
                foreach (var ms in _MSlinks)
                {
                    if (ms.maidKeepSlaveYotogi && ms.maidKeepSlaveYotogi.Visible)
                    {
                        ...
                        if (!ms.keepSI.CheckMoved(ms.maidKeepSlaveYotogi, ms.num_) &&
                            GameMain.Instance.CharacterMgr.GetMaidCount() > 0
        ...
        // 手と手のアタッチ
        static void AtccHand2HandR2(Maid master, Maid slave, Vector3 v3HandROffset, Vector3 v3HandROffsetRot,
            MsLinkConfig mscfg)
        {
            ...
            if (tgt)
            {
                ...
                {
                    IkXT.SetHandIKTarget(mscfg, "右手", master, slave, (int)TBody.SlotID.body, string.Empty, tgt,
                        v3HandROffset);
                }
                ...
                if (!mscfg.doIK159NewPointToDef && mscfg.doIK159RotateToHands)
                {
                    ...
                    Transform trh0 =
                        BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, GetHandBnR(master), true);
                    Transform trh =
                        BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnR(slave), true);
                    trh.rotation = trh0.rotation * Quaternion.Euler(v3HandROffsetRot);
                }
                else if (IkXT.IsNewIK && mscfg.doIK159RotateToHands /*Ik159.IsNewPointIK(slave, "右手")*/
                        ) // 手首角度のアタッチ v0027
                {
        ...
        static void AtccHand2HandL2(Maid master, Maid slave, Vector3 v3HandLOffset, Vector3 v3HandLOffsetRot,
            MsLinkConfig mscfg)
        {
            ...
            if (tgt)
            {
                ...
                if (IkXT.IsIkCtrlO117)
                {
                    IkXT.SetHandIKTarget(mscfg, "左手", master, slave, (int)TBody.SlotID.body, string.Empty, tgt,
                        v3HandLOffset);
                }
                ...
                if (!mscfg.doIK159NewPointToDef && mscfg.doIK159RotateToHands)
                {
                    ...
                    Transform trh0 =
                        BoneLink.BoneLink.SearchObjName(master.body0.m_Bones.transform, GetHandBnL(master), true);
                    Transform trh =
                        BoneLink.BoneLink.SearchObjName(slave.body0.m_Bones.transform, GetHandBnL(slave), true);
                    trh.rotation = Quaternion.Inverse(trh0.rotation) * Quaternion.Euler(v3HandLOffsetRot);
                }
                else if (IkXT.IsNewIK && mscfg.doIK159RotateToHands /*Ik159.IsNewPointIK(slave, "左手")*/
                        ) // 手首角度のアタッチ v0027
                {
        ...
        static void AtccHand1R(Maid slave, Maid tgtm, string atcpTgt, Vector3 v3HandOffset)
        {
            ...
            if (tgtm && !string.IsNullOrEmpty(atcpTgt))
            {
                ...
                {
                    var bonetgt = BoneLink.BoneLink.SearchObjName(tgtm.body0.m_Bones.transform,
                        atcpTgt.Remove(0, Defines.data.comboBonePrefix.Length), true);
		...
		bool bone2Atcp(Maid maid, string bonename)
		{
			...
			Transform[] t_bones =
 bodytmph.GetType().InvokeMember("m_bones", BindingFlags.GetField | BindingFlags.Instance | BindingFlags.DeclaredOnly | BindingFlags.NonPublic, null, bodytmph, null) as Transform[];
			if (t_bones == null)
				return false;
        ...
        public bool RenewBindVert(TBody.SlotID slotID, TMorph tm, string apname)
        {
            ...
            gaoo.m_bindposes = tm.GetType().InvokeMember("m_bindposes",
                BindingFlags.GetField | BindingFlags.Instance | BindingFlags.DeclaredOnly | BindingFlags.NonPublic,
                null, tm, null) as Matrix4x4[];
            gaoo.m_vTmpVert = tm.GetType().InvokeMember("m_vTmpVert",
                BindingFlags.GetField | BindingFlags.Instance | BindingFlags.DeclaredOnly | BindingFlags.NonPublic,
                null, tm, null) as Vector3[];
            ...
            if (slotID == TBody.SlotID.head)
            {
                ...
                Vector3 vector = Vector3.zero;
                vector += gaoo.m_bindposes[tAttachPoint.bw.boneIndex0].MultiplyPoint3x4(gaoo.m_vTmpVert[vidx]) *
                          tAttachPoint.bw.weight0;
                vector += gaoo.m_bindposes[tAttachPoint.bw.boneIndex1].MultiplyPoint3x4(gaoo.m_vTmpVert[vidx]) *
                          tAttachPoint.bw.weight1;
                vector += gaoo.m_bindposes[tAttachPoint.bw.boneIndex2].MultiplyPoint3x4(gaoo.m_vTmpVert[vidx]) *
                          tAttachPoint.bw.weight2;
                vector += gaoo.m_bindposes[tAttachPoint.bw.boneIndex3].MultiplyPoint3x4(gaoo.m_vTmpVert[vidx]) *
                          tAttachPoint.bw.weight3;
            ...
            else
            {
                //FixBlendValueより
                tm.BindVert[vidx] =
                    gaoo.m_bindposes[tAttachPoint.bw.boneIndex0].MultiplyPoint3x4(gaoo.m_vTmpVert[vidx]);
            }
        ...
        public class MsUtil
        {
            ...
            public static bool SelectSlave(int pageNum, int memNum)
            {
                ...
                else
                {
                    ...
                    {
                        if (p_mscfg.doKeepSlaveYotogi &&
                            XtMasterSlave.IsKeepScene() /*(vIsKaisouScene || bIsYotogiScene)*/)
            ...
            public static bool StartMsLink(int pageNum, bool Stop)
            {
                ...
                MsLinks ms_ = _MSlinks[pageNum];
                Maid slave = (ms_.mdSlave_No >= 0 && ms_.mdSlaves.Count() > ms_.mdSlave_No)
                    ? ms_.mdSlaves[ms_.mdSlave_No].mem
                    : null;
            ...
            /// <returns>成功ならTrue</returns>
            public static bool ConfigMsLink(int pageNum, bool StackPos, bool AutoCnkPos, bool AtcMsHands,
                bool CopyIkHands, bool PosSync, bool VoiceAndFacePlay)
            {
                if (pageNum < 0)
                    return false;
            ...
            public void MouthChange(Maid maid, int mode)
            {
                ...
                if (mode == 1 || mode == 2)
                {
                    if (TupValue < TupValue2)
                    {
                        ...
                        if (TupValue >= TupValue2)
                        {
                            TupValue2 = UnityEngine.Random.Range(0f, 60f) / 100f;
                        }
                    }
                    else
                    {
                    ...
                    if (ToutValue < ToutValue2)
                    {
                        ...
                        if (ToutValue >= ToutValue2)
                        {
                            ToutValue2 = UnityEngine.Random.Range(0f, 50f) / 100f;
                        }
                    }
                    else
                    {
                    ...
                    if (TopenValue < TopenValue2)
                    {
                        ...
                        if (TopenValue >= TopenValue2)
                        {
                            TopenValue2 = UnityEngine.Random.Range(0f, 40f) / 100f;
                        }
                    }
                    else
                    {
                ...
                if (mode == 3)
                {
                    if (TupValue < TupValue2)
                    {
                        ...
                        if (TupValue >= TupValue2)
                        {
                            TupValue2 = UnityEngine.Random.Range(0f, 40f) / 100f;
                        }
                    }
                    else
                    {
                    ...
                    if (ToutValue < ToutValue2)
                    {
                        ...
                        if (ToutValue >= ToutValue2)
                        {
                            ToutValue2 = UnityEngine.Random.Range(60f, 100f) / 100f;
                        }
                    }
                    else
                    {
                    ...
                    if (TopenValue < TopenValue2)
                    {
                        ...
                        if (TopenValue >= TopenValue2)
                        {
                            TopenValue2 = UnityEngine.Random.Range(0f, 60f) / 100f;
                        }
                    }
                    else
                    {
        ...
        public class AnimeState
        {
            ...
            public KeyValuePair<string, float> GetPlayingFN_withTime(TBody body0)
            {
                ...
                if (anim.IsPlaying(body0.LastAnimeFN))
                    return new KeyValuePair<string, float>(body0.LastAnimeFN,
                        anim[body0.LastAnimeFN].normalizedTime /*.normalizedTime*/); //再生中なら
                ...
                return new KeyValuePair<string, float>(m_LastGetMotion,
                    anim[m_LastGetMotion].normalizedTime /*.normalizedTime*/);
            }
        }
    }
...
namespace ExtensionMethods
{
    public static class ComExt
    {
        ...
        public class XtTMorph
        {
            ...
            FieldInfo fiBlendValues = typeof(TMorph).GetField("BlendValues",
        ...
        public static void GetAttachPoint(this TMorph tm, string apname, out Vector3 vWorldPos,
            out Quaternion qWorldRot)
        {
    ...
    public static class MyExtensions
    {
        ...
        public class IkXT
        {
            ...
            // com3d2 1.32~
            public static Type _typIKO132 =
                Assembly.Load("Assembly-CSharp").GetType("IKCtrlData")?.GetNestedType("IKSettingData");
            ...
            public static void SetHandIKRotate(string handName, Maid master, Maid slave, string boneTgtname,
            ...
            public static void SetHandIKTarget(XtMasterSlave.MsLinkConfig mscfg, string handName, Maid master,
                Maid slave, int slot_no, string attach_name, Transform target, Vector3 v3HandLOffset)
            {
                if (IsNewIK)
        ...
        public class IkInst
        {
            ...
            public virtual void IkClear(Maid tgt, List<string> listHand, XtMasterSlave.MsLinkConfig mscfg,
                int IkType = (-1))
            {
            }
            ...
            public virtual void SetHandIKRotate(string handName, Maid master, Maid slave, string boneTgtname,
                Vector3 v3HandLOffsetRot)
            {
            }
            ...
            public virtual void SetHandIKTarget(XtMasterSlave.MsLinkConfig mscfg, string handName, Maid master,
                Maid slave, int slot_no, string attach_name, Transform target, Vector3 v3HandLOffset)
            {
            }
        ...
        static void SetFI()
        {
            fiPointL = typeof(TBody).GetField("IkPointL",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
            fiPointR = typeof(TBody).GetField("IkPointR",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
            ...
            if (IkXT.IsIkCtrlO132)
            {
                ...
                _handNew.fiTgt = typ132.GetField("Target",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiMaid = typ132.GetField("TgtChara",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiAslot = typ132.GetField("Tgt_AttachSlot",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiAname = typ132.GetField("Tgt_AttachName",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiOffset = typ132.GetField("TgtOffset",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
            }
            else if (IkXT.IsIkCtrlO117)
            {
                ...
                _handNew.fiTgt = typL.GetField("Target",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiMaid = typL.GetField("TgtMaid",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiAslot = typL.GetField("Tgt_AttachSlot",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiAname = typL.GetField("Tgt_AttachName",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiOffset = typL.GetField("TgtOffset",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
            }
            ...
            else if (fiPointL == null && fiPointR == null)
            {
                ...
                boLegacy = true;
                _handR.fiTgt = typeof(TBody).GetField("tgtHandR",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handR.fiMaid = typeof(TBody).GetField("tgtMaidR",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handR.fiAslot = typeof(TBody).GetField("tgtHandR_AttachSlot",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handR.fiAname = typeof(TBody).GetField("tgtHandR_AttachName",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handR.fiOffset = typeof(TBody).GetField("tgtHandR_offset",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);

                _handL.fiTgt = typeof(TBody).GetField("tgtHandL",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handL.fiMaid = typeof(TBody).GetField("tgtMaidL",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handL.fiAslot = typeof(TBody).GetField("tgtHandL_AttachSlot",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handL.fiAname = typeof(TBody).GetField("tgtHandL_AttachName",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handL.fiOffset = typeof(TBody).GetField("tgtHandL_offset",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
            }
            else
            {
                ...
                _handNew.fiTgt = typL.GetField("HandTgt",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiMaid = typL.GetField("TgtMaid",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                _handNew.fiAslot = typL.GetField("Tgt_AttachSlot",
        ...
        public class SetIK
        {
            ...
            {
                get { return (Transform)fiR.fiTgt.GetValue(objR); }
                set { fiR.fiTgt.SetValue(objR, value); }
            }

            public Transform tgtHandL
            {
                get { return (Transform)fiL.fiTgt.GetValue(objL); }
                set { fiL.fiTgt.SetValue(objL, value); }
            }

            public Maid tgtMaidR
            {
                get { return fiR.fiMaid.GetValue(objR) as Maid; }
                set { fiR.fiMaid.SetValue(objR, value); }
            }

            public Maid tgtMaidL
            {
                get { return fiL.fiMaid.GetValue(objL) as Maid; }
                set { fiL.fiMaid.SetValue(objL, value); }
            }

            public Vector3 tgtHandR_offset
            {
                get { return (Vector3)fiR.fiOffset.GetValue(objR); }
                set { fiR.fiOffset.SetValue(objR, value); }
            }

            public Vector3 tgtHandL_offset
            {
                get { return (Vector3)fiL.fiOffset.GetValue(objL); }
                set { fiL.fiOffset.SetValue(objL, value); }
            }

            public int tgtHandR_AttachSlot
            {
                get { return (int)fiR.fiAslot.GetValue(objR); }
                set { fiR.fiAslot.SetValue(objR, value); }
            }

            public int tgtHandL_AttachSlot
            {
                get { return (int)fiL.fiAslot.GetValue(objL); }
                set { fiL.fiAslot.SetValue(objL, value); }
            }

            public string tgtHandR_AttachName
            {
                get { return fiR.fiAname.GetValue(objR) as string; }
                set { fiR.fiAname.SetValue(objR, value); }
            }

            public string tgtHandL_AttachName
            {
                get { return fiL.fiAname.GetValue(objL) as string; }
                set { fiL.fiAname.SetValue(objL, value); }
            }
        ...
        static FieldInfo fim_IkMgr = typeIKP != null
            ? typeIKP.GetField("m_IkMgr", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly)
            : null;

        static FieldInfo fiIkpointR = typeof(TBody).GetField("IkPointR",
            BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);

        static FieldInfo fiIkpointL = typeof(TBody).GetField("IkPointL",
            BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);

        static FieldInfo fiVechand = typeof(TBody.IKCMO).GetField("vechand",
            BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);

        static FieldInfo fiKnee_old = typeof(TBody.IKCMO).GetField("knee_old",
            BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);
        ...
        public static void owIkParam(this TBody tb, bool boR, Vector3 vechand, Vector3 knee_old)
        {
            ...
            {
                if (boR)
                    ikm = typeof(TBody).InvokeMember("ikRightArm",
                        BindingFlags.GetField | BindingFlags.NonPublic | BindingFlags.Instance |
                        BindingFlags.DeclaredOnly, null, tb, null);
                else
                    ikm = typeof(TBody).InvokeMember("ikLeftArm",
                        BindingFlags.GetField | BindingFlags.NonPublic | BindingFlags.Instance |
                        BindingFlags.DeclaredOnly, null, tb, null);
            }
        ...
        public static void setR_vechand(this TBody tb, Vector3 v3)
        {
            ...
            if (typeIKP != null)
            {
                ...
                ikm = typeof(TBody).InvokeMember("ikRightArm",
                    BindingFlags.GetField | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly,
                    null, tb, null);
        ...
        static MethodInfo miIkcmoProc1 = typeof(TBody.IKCMO).GetMethod("Proc",
            BindingFlags.Public | BindingFlags.Instance, null,
            new Type[] { typeof(Transform), typeof(Transform), typeof(Transform), typeof(Vector3), typeof(Vector3) },
            null);

        static MethodInfo miIkcmoProc =
            typeof(TBody.IKCMO).GetMethod("Proc", BindingFlags.Public | BindingFlags.Instance);
        ...
        public static void IKCmoProc(this TBody body, bool handRight, Transform bone, Vector3 tgt, Vector3 offset)
        {
            ...
            switch (miIkcmoProc.GetParameters().Length)
            {
                case 5:
                    ...
                    break;
                case 6:
                    miIkcmoProc.Invoke(cmo,
                        new object[] { bone.parent.parent, bone.parent, bone, tgt, offset, ikctrl });
                    break;
            }
        }
...
namespace BoneLink
{
    public class BoneLink
    {
        ...
        static Dictionary<Transform, Dictionary<string, Transform>> SearchObjCache =
            new Dictionary<Transform, Dictionary<string, Transform>>();
        ...
        public void Try(Maid master, Maid slave, bool enabled, bool org_pelvis, bool org_clit, Vector3 v3StackOffset,
            XtMasterSlave.v3Offsets v3ofs, XtMasterSlave.MsLinkConfig mcfg)
        {
            if (!master || !slave)
                return;
            ...
            else if (!master.boMAN)
            {
                ...
                foreach (var bone in Defines.data.MaidBones)
                {
                    try
                    {
                        ...
                        string searchName =
                            Regex.Replace(bone, @"(.*)@(.*)$",
                                @"$1"); // If there is no "@" in the name, it won't be changed
                ...
                foreach (var bone in Defines.data.ManBones)
                {
                    try
                    {
                        ...
                        {
                            SearchObjName(man_bone_tr, bone, true).localRotation =
                                new Quaternion(q0[0], q0[1], q0[2], q0[3]); //(q.x, q.y, q.z, q.w);
                        }
                    }
                    catch (Exception e)
                    {
                ...
                try
                {
                    ...
                    if (org_clit)
                    {
                        trManBip.position += (SearchObjName(maid_bone_tr, "Bip01 Pelvis", true).position -
                                              gotr_mas.position)
                                             - (SearchObjName(man_bone_tr, "chinkoCenter", true).position -
                                                gotr_slv.position);
                    ...
                    else if (org_pelvis)
                    {
                        trManBip.position += (SearchObjName(maid_bone_tr, "Bip01 Pelvis", true).position -
                                              gotr_mas.position)
                                             - (SearchObjName(man_bone_tr, "ManBip Pelvis", true).position -
                                                gotr_slv.position);
            ...
            else
            {
                ...
                foreach (var bone in Defines.data.ManBones)
                {
                    try
                    {
                        ...
                        string searchName =
                            Regex.Replace(bone, @"(.*)@(.*)$",
                                @"$1"); // If there is no "@" in the name, it won't be changed
                        Quaternion q = SearchObjName(man_bone_tr, searchName, true).localRotation;
                        dicManPose[bone] = q;
                    }
                    catch (Exception e)
                    {
                ...
                foreach (var bone in Defines.data.MaidBones)
                {
                    try
                    {
                        ...
                        {
                            SearchObjName(maid_bone_tr, bone, true).localRotation =
                                new Quaternion(q0[0], q0[1], q0[2], q0[3]); //(q.x, q.y, q.z, q.w);
                        }
                    }
                    catch (Exception e)
                ...
                //ボーン基準位置合わせ | Bone reference alignment
                try
                {
                    Transform trManBip = SearchObjName(man_bone_tr, "ManBip", true);
                    Transform trBip01 = SearchObjName(maid_bone_tr, "Bip01", true);
...
#endif
                        trHuta.localRotation = SearchObjName(man_bone_tr, "chinkoCenter", true).localRotation * q *
                    ...
                    else
                    {
                        if (!trHuta)
                        {
                            ...
                            go.transform.localRotation =
                                new Quaternion(0.5323541f, -0.4653954f, -0.4654013f, 0.5323607f);
                    ...
                    if (org_clit)
                    {
                        trBip01.position += (SearchObjName(man_bone_tr, "chinkoCenter", true).position -
                                             gotr_mas.position)
                                            - (SearchObjName(maid_bone_tr, "Bip01 Pelvis", true).position -
                                               gotr_slv.position);
...
#endif
                        trBip01.position += (SearchObjName(man_bone_tr, "ManBip Pelvis", true).position -
                                             gotr_mas.position)
                                            - (SearchObjName(maid_bone_tr, "Bip01 Pelvis", true).position -
                                               gotr_slv.position);
        ...
        public static Vector3 TryPosSp(Maid master, Maid slave, bool enabled, bool org_pelvis, bool org_clit,
            bool PosSyncModeV2, string tgtBonename,
        ...
        static Dictionary<string, Quaternion> maid2man(Dictionary<string, Quaternion> dicBones)
        {
            ...
            {
                string name = bn.Key;
                if (!name.Contains("Bip01") && !name.Contains("@")) name = ""; //v0020 不要なボーンを除く
                ...
                if (name != "")
                {
                    ...
                    string destName =
                        Regex.Replace(name, @"(.*)@(.*)$",
                            @"$2"); // If there is no "@" in the name, it won't be changed
                    dest[destName] = bn.Value;
                }
            }
            ...
            {
                {
                    ...
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger0").gameObject)
                },
                {
                    IKManager.BoneType.Finger0_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger01").gameObject)
                },
                {
                    IKManager.BoneType.Finger0_1_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger02").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger1").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger11").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_1_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger12").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger2").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger21").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_1_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger22").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger3").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger31").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_1_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger32").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger4").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger41").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_1_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightArmFinger,
                        maid.body0.GetBone("Bip01 R Finger42").gameObject)
                },
                {
                    IKManager.BoneType.Finger0_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger0").gameObject)
                },
                {
                    IKManager.BoneType.Finger0_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger01").gameObject)
                },
                {
                    IKManager.BoneType.Finger0_1_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger02").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger1").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger11").gameObject)
                },
                {
                    IKManager.BoneType.Finger1_1_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger12").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger2").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger21").gameObject)
                },
                {
                    IKManager.BoneType.Finger2_1_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger22").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger3").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger31").gameObject)
                },
                {
                    IKManager.BoneType.Finger3_1_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger32").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger4").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger41").gameObject)
                },
                {
                    IKManager.BoneType.Finger4_1_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftArmFinger,
                        maid.body0.GetBone("Bip01 L Finger42").gameObject)
                },
                {
                    IKManager.BoneType.Toe0_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe2").gameObject)
                },
                {
                    IKManager.BoneType.Toe0_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe21").gameObject)
                },
                {
                    IKManager.BoneType.Toe1_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe1").gameObject)
                },
                {
                    IKManager.BoneType.Toe1_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe11").gameObject)
                },
                {
                    IKManager.BoneType.Toe2_Root_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe0").gameObject)
                },
                {
                    IKManager.BoneType.Toe2_0_R,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 R Toe01").gameObject)
                },
                {
                    IKManager.BoneType.Toe0_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.RightLegFinger,
                        maid.body0.GetBone("Bip01 L Toe2").gameObject)
                },
                {
                    IKManager.BoneType.Toe0_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftLegFinger,
                        maid.body0.GetBone("Bip01 L Toe21").gameObject)
                },
                {
                    IKManager.BoneType.Toe1_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftLegFinger,
                        maid.body0.GetBone("Bip01 L Toe1").gameObject)
                },
                {
                    IKManager.BoneType.Toe1_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftLegFinger,
                        maid.body0.GetBone("Bip01 L Toe11").gameObject)
                },
                {
                    IKManager.BoneType.Toe2_Root_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftLegFinger,
                        maid.body0.GetBone("Bip01 L Toe0").gameObject)
                },
                {
                    IKManager.BoneType.Toe2_0_L,
                    new KeyValuePair<IKManager.BoneSetType, GameObject>(IKManager.BoneSetType.LeftLegFinger,
                        maid.body0.GetBone("Bip01 L Toe01").gameObject)
                }
            };
        }
    }
...
namespace Defines
{
    ...
    */
    ...
    class
        data // luvoid - changed this so custom bones could be added to bones list by calling AppendTo[Maid/Man]Bones() or by adding it to xtCustom[Maid/Man]Bones.txt
    {
        public readonly static string comboBonePrefix = "* ";

        public readonly static string[] defaultMaidBones =
        {
            "Bip01",
            "Bip01 Head",
            "Bip01 Neck",
        ...
        public readonly static string[] defaultManBones =
        {
            "ManBip",
            "ManBip Head",
            "ManBip Neck",
        ...
        public static void AppendToMaidBones(string[] boneNames)
        {
            _MaidBones.AddRange(boneNames.ToList());
        }

        public static void AppendToManBones(string[] boneNames)
        {
            _ManBones.AddRange(boneNames.ToList());
        }

        public static Exception AppendToMaidBones(string fileName, bool fullPath = false)
        {
            string[] boneNames;
            try
            {
                boneNames = GetBoneNamesFromFile(fileName, fullPath);
            }
            catch (Exception e)
            {
                Console.WriteLine("XtMS: AppendToMaidBones file error. " + e);
                return e;
            }

            AppendToMaidBones(boneNames);
            return null;
        }

        public static Exception AppendToManBones(string fileName, bool fullPath = false)
        {
            string[] boneNames;
            try
            {
            ...
            {
                Console.WriteLine("XtMS: AppendToManBones file error. " + e);
                return e;
            }

            AppendToManBones(boneNames);
            return null;
        }
        ...
        public static string[] MaidBones
        {
            get
            {
                ...
                {
                    AppendToMaidBones(defaultMaidBones);
        ...
        public static string[] ManBones
        {
            get
            {
                if (_ManBones.Count <= 0)
                {
                    AppendToManBones(defaultManBones);
                    AppendToManBones("xtCustomManBones.txt");
                }

                return _ManBones.ToArray();
            }
        }
    }
}
...
namespace VYMModule
{
    ...
    class cnv2csv
    {
        static readonly string[] levelTypes = new string[]
        {
            /*"10",*/ "20", "30", "40"
        };
#if COM3D2
        static readonly string[] maidTypes = new string[]
            { "Pure", "Pride", "Cool", "Yandere", "Anesan", "Genki", "Sadist", "Muku", "Majime", "Rindere", };
#else
		static readonly string[] maidTypes =
 new string[] { "Pure", "Pride", "Cool", "Yandere", "Anesan", "Genki", "Sadist", };
#endif
        static readonly string[] voiceTypes = new string[]
            { "sLoopVoice{0}{1}Vibe", "sLoopVoice{0}{1}Fera", "sOrgasmVoice{0}{1}Vibe", "sOrgasmVoice{0}{1}Fera", };
        ...
        public static void SaveVoiceCsvFile()
        {
            ...
            try
            {
                ...
                {
                    sw.WriteLine(
                        "VoiceType,State(20=Low|30=High|40=Rest),Personal,Level(0~3=ExciteLv|4=Stun|-1=N/A),Files...");
                    toCsv(sw);
                    sw.Flush();
                    sw.Close();
                }
                    ...
                    voiceTypes.ToList().ForEach(z =>
                    {
                        ...
                        try
                        {
                            ...
                            void outputLine(string[] vList, int state)
                            {
                                ...
                                {
                                    string.Format(z, "", "").Substring(1).Replace("Vibe", ""), x, y, state.ToString()
                                };
                                line.AddRange(vList);
                    ...
                    voiceTypes.ToList().ForEach(z =>
                    {
                        ...
                        try
                        {
                            ...
                            void outputLine(string[] vList, int state)
                            {
                                List<string> line = new List<string>
                                {
                                    string.Format(z, "", "").Substring(1).Replace("Vibe", ""), x, "Custom" + y,
                                    state.ToString()
                                };
                                line.AddRange(vList);
                                sw.WriteLine(string.Join(",", line.ToArray()));
    ...
    class NewVoiceTable
    {
        ...
        public class VoiceSet
        {
            ...
            public VoiceSet(string type, string state, string personal, string level, string[] files)
                : this((VoiceType)Enum.Parse(typeof(VoiceType), type, true), int.Parse(state), personal,
                    int.Parse(level), files)
            {
            }

            public VoiceSet(string type, string state, string personal, string level)
                : this((VoiceType)Enum.Parse(typeof(VoiceType), type, true), int.Parse(state), personal,
                    int.Parse(level), new string[0])
            {
            }
        }
        ...
        public static VoiceSet GetVoiceSet(NewVoiceTable.VoiceSet.VoiceType VoiceType, int State, string Personal,
            int Level)
        {
    ...
    public class VymModule
    {
        ...
        //複数メイド対応型
        private static Dictionary<int, Dictionary<Maid, int>> iRandomVoiceBackup =
            new Dictionary<int, Dictionary<Maid, int>>
            {
        ...
        public class VibeYourMaidConfig
        {
            ...
            public string[] sFaceAnimeStun = new string[]
                { "絶頂射精後１", "興奮射精後１", "エロメソ泣き", "エロ痛み２", "エロ我慢３", "引きつり笑顔", "エロ通常３", "泣き" };
        }
        ...
        public class VoiceTableLegacy
        {
            ...
            public string[][] sOrgasmVoice30PrideVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s0_09067.ogg", "s0_09068.ogg", "s0_09069.ogg", "s0_09071.ogg", "s0_09085.ogg", "s0_09086.ogg",
                    "s0_09087.ogg", "s0_09091.ogg"
                },
                new string[] { "s0_01898.ogg", "s0_01899.ogg", "s0_01902.ogg", "s0_01900.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30CoolVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s1_09344.ogg", "s1_09370.ogg", "s1_09371.ogg", "s1_09372.ogg", "s1_09374.ogg", "s1_09398.ogg",
                    "s1_09392.ogg", "s1_09365.ogg"
                },
                new string[] { "s1_03223.ogg", "s1_03246.ogg", "s1_03247.ogg", "s1_03210.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30PureVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s2_09047.ogg", "s2_09048.ogg", "s2_09049.ogg", "s2_09050.ogg", "s2_09051.ogg", "s2_09066.ogg",
                    "s2_09069.ogg", "s2_09073.ogg"
                },
                new string[] { "s2_01478.ogg", "s2_01477.ogg", "s2_01476.ogg", "s2_01475.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30YandereVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s3_05657.ogg", "s3_05658.ogg", "s3_05659.ogg", "s3_05660.ogg", "s3_05661.ogg", "s3_05678.ogg",
                    "s3_05651.ogg", "s3_05656.ogg"
                },
                new string[] { "s3_02908.ogg", "s3_02950.ogg", "s3_02923.ogg", "s3_02932.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30AnesanVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s4_11684.ogg", "s4_11677.ogg", "s4_11680.ogg", "s4_11683.ogg", "s4_11661.ogg", "s4_11659.ogg",
                    "s4_11654.ogg", "s4_11660.ogg"
                },
                new string[] { "s4_08348.ogg", "s4_08354.ogg", "s4_08365.ogg", "s4_08374.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30GenkiVibe = new string[][]
            {
                ...
                new string[]
                {
                    "s5_18379.ogg", "s5_18380.ogg", "s5_18382.ogg", "s5_18384.ogg", "s5_18385.ogg", "s5_18400.ogg",
                    "s5_18402.ogg", "s5_18119.ogg"
                },
                new string[] { "s5_04264.ogg", "s5_04258.ogg", "s5_04256.ogg", "s5_04255.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30SadistVibe = new string[][]
            {
                ...
                new string[]
                {
                    "S6_09048.ogg", "S6_01984.ogg", "S6_01988.ogg", "S6_01991.ogg", "S6_02000.ogg", "S6_01996.ogg",
                    "S6_01997.ogg", "S6_01998.ogg", "S6_01999.ogg", "S6_02001.ogg", "s6_05796.ogg", "s6_05797.ogg",
                    "s6_05798.ogg", "s6_05799.ogg", "s6_05800.ogg", "s6_05801.ogg"
                },
                new string[] { "s6_01744.ogg", "s6_02700.ogg", "s6_02450.ogg", "s6_02357.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30SadistFera = new string[][]
            {
                ...
                new string[]
                    { "S6_02420.ogg", "S6_08109.ogg", "S6_08112.ogg", "S6_08114.ogg", "s6_02404.ogg", "s6_02405.ogg" },
                new string[] { "S6_28832.ogg", "s6_02403.ogg", "S6_28835.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30Custom1 = new string[][]
            {
                ...
                new string[]
                {
                    "N0_00456.ogg", "N0_00457.ogg", "N0_00458.ogg", "N0_00534.ogg", "N0_00288.ogg", "N0_00292.ogg",
                    "N0_00293.ogg"
                },
                new string[] { "N0_00424.ogg", "N0_00459.ogg", "N0_00503.ogg", "N0_00508.ogg", "N0_00534.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30Custom2 = new string[][]
            {
                ...
                new string[]
                {
                    "N7_00284.ogg", "N7_00291.ogg", "N7_00293.ogg", "N7_00294.ogg", "N7_00295.ogg", "N7_00275.ogg",
                    "n7_00295.ogg"
                },
                new string[] { "N7_00251.ogg", "N7_00267.ogg", "N7_00275.ogg", "N7_00276.ogg", "N7_00280.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30Custom3 = new string[][]
            {
                ...
                new string[]
                {
                    "N1_00179.ogg", "N1_00180.ogg", "N1_00198.ogg", "N1_00199.ogg", "N1_00205.ogg", "N1_00217.ogg",
                    "N1_00333.ogg"
                },
                new string[] { "N1_00179.ogg", "N1_00180.ogg", "N1_00200.ogg", "N1_00204.ogg", "N1_00209.ogg" }
            };
            ...
            public string[][] sOrgasmVoice30Custom4 = new string[][]
            {
                ...
                new string[]
                {
                    "N3_00376.ogg", "N3_00194.ogg", "N3_00195.ogg", "N3_00197.ogg", "N3_00203.ogg", "N3_00328.ogg",
                    "N3_00330.ogg", "N3_00379.ogg"
                },
                new string[] { "N3_00193.ogg", "N3_00194.ogg", "N3_00195.ogg", "N3_00330.ogg", "N3_00378.ogg" }
            };
            ...
            //　性格別声テーブル　停止時
            public string[] sLoopVoice40PrideVibe = new string[]
                { "S0_01967.ogg", "S0_01967.ogg", "S0_01968.ogg", "S0_01969.ogg", "S0_01969.ogg" };

            public string[] sLoopVoice40CoolVibe = new string[]
                { "S1_03264.ogg", "S1_03264.ogg", "S1_03265.ogg", "S1_03266.ogg", "S1_03266.ogg" };

            public string[] sLoopVoice40PureVibe = new string[]
                { "s2_01491.ogg", "s2_01491.ogg", "s2_01492.ogg", "s2_01493.ogg", "s2_01493.ogg" };

            public string[] sLoopVoice40YandereVibe = new string[]
                { "S3_02964.ogg", "S3_02964.ogg", "S3_02965.ogg", "S3_02966.ogg", "S3_02966.ogg" };

            public string[] sLoopVoice40AnesanVibe = new string[]
                { "s4_08424.ogg", "s4_08426.ogg", "s4_08427.ogg", "s4_08428.ogg", "s4_08428.ogg" };

            public string[] sLoopVoice40GenkiVibe = new string[]
                { "s5_04127.ogg", "s5_04129.ogg", "s5_04131.ogg", "s5_04134.ogg", "s5_04134.ogg" };

            public string[] sLoopVoice40SadistVibe = new string[]
                { "s6_02477.ogg", "s6_02478.ogg", "s6_02479.ogg", "s6_02481.ogg", "s6_02480.ogg" };

#if COM3D2
            // オダメ追加分
            //案1 public string[] sLoopVoice40MukuVibe = new string[] { "H0_00337.ogg", "H0_00352.ogg", "H0_00338.ogg", "H0_00354.ogg", "H0_00339.ogg" };
            public string[] sLoopVoice40MukuVibe = new string[]
                { "H0_15027.ogg", "H0_00352_vd.ogg", "H0_00338.ogg", "H0_00354.ogg", "H0_00339.ogg" };

            //案1 public string[] sLoopVoice40MajimeVibe = new string[] { "H1_00509.ogg", "H1_00524.ogg", "H1_00510.ogg", "H1_00511.ogg", "H1_00526.ogg" };
            public string[] sLoopVoice40MajimeVibe = new string[]
            {
                "H1_00525_vd.ogg" /*"H1_08954.ogg"*/, "H1_00524_vd.ogg", "H1_00510.ogg", "H1_00526.ogg", "H1_00526.ogg"
            };

            //案1 public string[] sLoopVoice40RindereVibe = new string[] { "H2_00311.ogg", "H2_00326.ogg", "H2_00327.ogg", "H2_00313.ogg", "H2_00328.ogg" };
            public string[] sLoopVoice40RindereVibe = new string[]
                { "H2_00326_vd.ogg", "H2_00326_vd.ogg", "H2_00327.ogg", "H2_00328.ogg", "H2_00313.ogg" };
#endif

            public string[] sLoopVoice40Custom1 = new string[]
                { "N0_00460.ogg", "N0_00460.ogg", "N0_00460.ogg", "N0_00460.ogg", "N0_00460.ogg" };

            public string[] sLoopVoice40Custom2 = new string[]
                { "N7_00277.ogg", "N7_00277.ogg", "N7_00277.ogg", "N7_00277.ogg", "N7_00277.ogg" };

            public string[] sLoopVoice40Custom3 = new string[]
                { "N1_00382.ogg", "N1_00382.ogg", "N1_00382.ogg", "N1_00382.ogg", "N1_00382.ogg" };

            public string[] sLoopVoice40Custom4 = new string[]
                { "N3_00205.ogg", "N3_00205.ogg", "N3_00205.ogg", "N3_00205.ogg", "N3_00205.ogg" };
        }
        ...
        public static void MaidVoicePlay(Maid vm, /*int Num, */int iExcite, XtMasterSlave.AnimeState.State motionState,
            bool boMotionChanged, bool faceAnimeEnabled, bool VoiceEnabled, bool modeVYM, bool modeManual,
            XtMasterSlave.MsLinkConfig msCfg, ref bool VoicePlaying)
        {
            ...
            else if (cfg.vExciteLevelThresholdV1 * 60 <= iCurrentExcite &&
                     iCurrentExcite < cfg.vExciteLevelThresholdV2 * 60)
            {
                vExciteLevel = 2;
            }
            else if (cfg.vExciteLevelThresholdV2 * 60 <= iCurrentExcite &&
                     iCurrentExcite < cfg.vExciteLevelThresholdV3 * 60)
            {
                vExciteLevel = 3;
            }
            ...
            if (modeVYM && !modeManual)
            {
                ...
                if (msCfg.doVoiceAndFacePlayOnVYM_Zeccho)
                {
                    ...
                    {
                        OrgasmVoice = (VYM.API.obj2int(VYM.API.GetVYM_Value(VYM.API.VYM_IO_ID.i_OrgasmVoice)) >= 1)
                            ? 1
                            : 0;
                    }
                    ...
                    if (OrgasmVoice != 0)
                    {
                        ...
                        if (vm.AudioMan.audiosource.loop ||
                            (!vm.AudioMan.audiosource.loop && !vm.AudioMan.audiosource.isPlaying))
            ...
            if (!vm.AudioMan.audiosource.loop && !vm.AudioMan.audiosource.isPlaying ||
                (VoiceEnabled && !VoicePlaying)) //→ループ音声停止中も割り込みを入れてみる(VoiceEnabled判定追加v0025)
            {
                // 一回再生音声が停止中
                /*m_iLoopWaitCnt++;
            ...
            if (OrgasmVoice != 0)
            {
                ...
                while (iRandomVoiceBackup[vi].ContainsKey(vm) && iRandomVoice == iRandomVoiceBackup[vi][vm] &&
                       VoiceList.Length > 1)
                {
		...
		private void checkBlowjobing(Maid m, int Num)
		{
			...
			if (maid)
			{
				...
				if (sLastAnimeFileName != null)
				{
					...
					if (bIsBlowjobing[Num] == 1 && cfgw.MouthKissEnabled) MouthMode[Num] =
 bIsBlowjobing[Num]; //1の時はキスが有効なら切り替える
					if (bIsBlowjobing[Num] == 2 && cfgw.MouthFeraEnabled) MouthMode[Num] =
 bIsBlowjobing[Num]; //2の時はフェラが有効なら切り替える
            ...
            try
            {
                if (m_aryAudioMan == null)
                {
                    ...
                    object objAudioSeBufSe = GameMain.Instance.SoundMgr.GetType().InvokeMember("m_AudioSeBufSe",
                        BindingFlags.GetField | BindingFlags.Instance | BindingFlags.DeclaredOnly |
                        BindingFlags.NonPublic, null, GameMain.Instance.SoundMgr, null);

                    if (objAudioSeBufSe != null)
                    {
                        object o = objAudioSeBufSe.GetType().InvokeMember("m_aryAudioMan",
                            BindingFlags.GetField | BindingFlags.Instance | BindingFlags.NonPublic, null,
                            objAudioSeBufSe, null);
...
namespace VYM
{
    ...
    public static class API
    {
            ...
            try
            {
                if (objVYM == null)
                {
                    ...
                    if (objVYM != null)
                    {
                        ...
                        mi_api_entry = objVYM.GetType().GetMethod("API_Entry",
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                        //if (Assembly.LoadFile(sPlugin).GetTypes().Any(t => t == typeof(VYM_IO_ID)))
                        if (mi_api_entry != null)
                        {
                            dlg_API_Entry = (delegate_API_Entry)Delegate.CreateDelegate(typeof(delegate_API_Entry),
                                objVYM, mi_api_entry, false);
        ...
        public static int GetPluginEnabled()
        {
            ...
            {
                UnityEngine.Debug.Log("GetPluginEnabled Error:" + e);
            }
        ...
        public static object GetVYM_Value(API.VYM_IO_ID id /*, bool PluginEnabled*/)
        {
            ...
            {
                UnityEngine.Debug.Log("GetVYM_Value Error:" + e);
            }
