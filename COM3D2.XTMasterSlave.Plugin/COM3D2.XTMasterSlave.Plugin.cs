				...
				if (cfg_.doStackSlave)
				{
					...
					if (cfg_.doStackSlave_PosSyncMode && cfg_.doStackSlave_PosSyncModeSp && !string.IsNullOrEmpty(cfg_.doStackSlave_PosSyncModeSp_TgtBone))
					{
						//if (cfg_.doStackSlave_PosSyncModeSp && !string.IsNullOrEmpty(cfg_.doStackSlave_PosSyncModeSp_TgtBone))
						{
							...
							if (!bnames.Contains(cfg_.doStackSlave_PosSyncModeSp_TgtBone)) // TODO just check if it exists in the armature to support custom bones
							{
			...
			else if (!master.boMAN)
			{
				...
				foreach (var bone in Defines.data.MaidBones)
				{
					try
					{
						...
						string searchName = Regex.Replace(bone, @"(.*)@(.*)$", @"$1"); // If there is no "@" in the name, it won't be changed
		...
		static Dictionary<string, Quaternion> maid2man(Dictionary<string, Quaternion> dicBones)
		{
			...
			foreach (var bn in dicBones)
			{
				...
				if (name != "") {
					...
					// luvoid - added Regex statement to support source@destination bones
					string destName = Regex.Replace(name, @"(.*)@(.*)$", @"$2"); // If there is no "@" in the name, it won't be changed
...
namespace Defines
{
	...
	*/
	...
	class data // luvoid - changed this so custom bones could be added to bones list by calling AppendTo[Maid/Man]Bones() or by adding it to xtCustom[Maid/Man]Bones.txt
	{
		public readonly static string comboBonePrefix = "* ";
