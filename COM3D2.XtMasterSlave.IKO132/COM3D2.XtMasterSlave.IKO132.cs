            ...
            listHand.ToList().ForEach(h =>
            {
                ...
                listTypes.ForEach(t =>
                {
                    ...
                    if (IkXT.IsIkCtrlO117)
                    {
                        ...
                        //ctrl.Detach(t); // they change this in protected in 2.39
                        MethodInfo detachMethod = ctrl.GetType().GetMethod(
                            "Detach",
                            BindingFlags.Instance | BindingFlags.NonPublic,
                            null,
                            new Type[] { typeof(IKCtrlData.IKAttachType) },
                            null
                        );
                        if (detachMethod != null)
                        {
                            detachMethod.Invoke(ctrl, new object[] { t });
                        }
